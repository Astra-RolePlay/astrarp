...
function OnEmotePlay(EmoteName, textureVariation)
    if timer <= 5 then return TriggerEvent("QBCore:Notify", "Ateş ettiğin için "..(6-timer).." saniye sonra animasyon yapabilirsin", "error") end
	if QBCore.Functions.GetPlayerData().metadata.isdead then QBCore.Functions.Notify('Ölüyken animasyon kullanamazsın!', 'error') return end
	if IsEntityInAir(PlayerPedId()) then QBCore.Functions.Notify('Havadayken Animasyon Kullanamazsın', 'error') return end
	if IsPedInAnyVehicle(PlayerPedId()) then QBCore.Functions.Notify('Araç içerisinde animasyon kullanamazsın!', 'error') return end
	if exports['qb-smallresources']:isInRagdoll() then QBCore.Functions.Notify("Baygınken animasyon kullanamazsın!", "error") return end
