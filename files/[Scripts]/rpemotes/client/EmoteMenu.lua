        ...
        sharemenu.OnItemSelect = function(sender, item, index)
            if ShareTable[index] ~= 'none' then
                ...
                if (distance ~= -1 and distance < 3) then
                    ...
                    QBCore.Functions.Notify(Config.Languages[lang]['sentrequestto'] .. GetPlayerName(target), "inform")
                else
                    QBCore.Functions.Notify(Config.Languages[lang]['nobodyclose'], "error")
                end
            end
        end
    ...
    function EmoteMenuSearch(lastMenu)
        ...
        if input ~= nil then
            ...
            if #results > 0 then
                ...
                searchMenu.OnItemSelect = function(sender, item, index)
                    ...
                    if favEnabled and IsControlPressed(0, 21) then
                        if data.table ~= "Shared" then
                            ...
                            QBCore.Functions.Notify("(" .. firstToUpper(data.name) .. Config.Languages[lang]['newsetemote'], "inform")
                        else
                            QBCore.Functions.Notify(Config.Languages[lang]['searchcantsetfav'], "inform")
                        end
                    ...
                    elseif data.table == "Shared" then
                        ...
                        if (distance ~= -1 and distance < 3) then
                            ...
                            QBCore.Functions.Notify(Config.Languages[lang]['sentrequestto'] .. GetPlayerName(target), "success")
                        else
                            QBCore.Functions.Notify(Config.Languages[lang]['nobodyclose'], "inform")
                        end   
                    end
                end
                ...
                QBCore.Functions.Notify("'"..input.."'"..Config.Languages[lang]['searchnoresult'], "error")
            end
        end
    end
end
...
function AddWalkMenu(menu)
    ...
    WalkInjured = NativeUI.CreateItem("Yaralı", "/walk (yaralı)")
