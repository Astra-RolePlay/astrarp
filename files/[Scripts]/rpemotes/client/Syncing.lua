        ...
        if #args > 0 then
            ...
            if (distance ~= -1 and distance < 3) then
                if RP.Shared[emotename] ~= nil then
                    ...
                    GetPlayerName(target) .. " ~w~(~g~" .. ename .. "~w~)", "inform")
                else
                    QBCore.Functions.Notify("'" .. emotename .. "' " .. Config.Languages[lang]['notvalidsharedemote'] .. "", "error")
                end
            else
...
AddEventHandler("ClientEmoteRequestReceive", function(emotename, etype)
    ...
    QBCore.Functions.Notify(Config.Languages[lang]['doyouwanna'].. remote ..")", "inform")
end)
...
Citizen.CreateThread(function()
    while true do
        ...
        elseif IsControlJustPressed(1, 182) and isRequestAnim then
            QBCore.Functions.Notify(Config.Languages[lang]['refuseemote'], "error")
            isRequestAnim = false
        end
    end
end)
