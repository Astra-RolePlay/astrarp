...
DoAcid = function(time)
  ...
  InitCubes()
  Wait(10000)
  ExecuteCommand('do Yavaş yavaş Mario\'yu görmeye başlar.')
  local step = 0
  ...
  SetPedIsDrunk(PlayerPedId(), true)
  SetPedMovementClipset(PlayerPedId(), "move_m@drunk@verydrunk", false)
  ...
  MarioZOff = -20.0
  MarioZAdd = 0.01
  TriggerEvent('QBCore:Notify', "Mario artık senin rüyalarında gezen bir arkadaşın.", 'success')
end
...
RegisterNetEvent("kfzeu:Acid")
AddEventHandler("kfzeu:Acid", function(time)
    QBCore.Functions.Progressbar("use_acid", "Hap Atılıyor", 3000, false, true, {
    ...
    disableCarMovement = false,
        ...
        TriggerServerEvent("acid:sil")
        TriggerEvent("inventory:client:ItemBox", QBCore.Shared.Items["lsd"], "remove")
        TriggerEvent('QBCore:Notify', "Olası durumda epilepsiniz var ise kullanmayınız.", 'error')
        Wait(4000)
        DoAcid(200000)
    end, function() -- Cancel
        StopAnimTask(ped, "mp_suicide", "pill", 1.0)
