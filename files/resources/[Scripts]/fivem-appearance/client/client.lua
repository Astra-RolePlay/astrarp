...
RegisterNetEvent('qb-clothes:client:CreateFirstCharacter', function()
    ...
    local pos = vector3(-763.22, 329.09, 198.60)
    -- FreezeEntityPosition(ped, true)
    SetEntityCoords(ped, pos.x, pos.y, pos.z)
    SetEntityHeading(ped, 181.00)
    ...
    Citizen.CreateThread(function()
        while firstConnect do
            ...
            if not IsEntityPlayingAnim(PlayerPedId(), "anim@amb@clubhouse@mini@darts@", "wait_idle", 3) then
                -- RequestAnimDict('anim@amb@clubhouse@mini@darts@', function() -- animasyon oynatma
        ...
        firstConnect = false
        TriggerServerEvent("qb-clothing:returnBucket")
        local playerPed = PlayerPedId()
        DoScreenFadeOut(1000)
        while not IsScreenFadedOut() do Citizen.Wait(100) end
        SetEntityCoords(playerPed, -1037.78, -2737.84, 19.17)
        SetEntityHeading(playerPed, 325.81)
        Citizen.Wait(1000)
        DoScreenFadeIn(1000)
    ...
    QBCore.Functions.GetPlayerData(function(pd)
        ...
        exports[resourceName]:startPlayerCustomization(function(appearance)
            if (appearance) then
                ...
                TriggerEvent('dream-hud:minimap-kapat')
                ResetRechargeMultipliers()
                firstConnect = false
...
function OpenShop(config, isPedMenu, shopType)
    QBCore.Functions.TriggerCallback("fivem-appearance:server:hasMoney", function(hasMoney, money)
        if not hasMoney and not isPedMenu then
            QBCore.Functions.Notify("Yeterli para yok. Gerekli miktar $" .. money, "error")
            TriggerEvent('dream-hud:minimap-kapat')
            return
        end
        ...
        exports[resourceName]:startPlayerCustomization(function(appearance)
            if appearance then
                ...
                QBCore.Functions.Notify("Özelleştirme İptal Edildi")
                TriggerEvent('dream-hud:minimap-kapat')
            end
        end, config)
    end, shopType)
...
function OpenMenu(isPedMenu, backEvent, menuType, menuData)
    ...
    local outfitMenuItems = {{
        ...
        header = "Yeni Kıyafet Kaydet",
        txt = "Daha sonra kullanmak üzere yeni bir kıyafet kaydedin",
        params = {
            event = "fivem-appearance:client:saveOutfit"
        }
    }, {
        header = "Kıyafetini Sil",
        txt = "Beğenmediğin bir kıyafet var ise silebilirsin",
        params = {
    ...
    if menuType == "default" then
        ...
        menuItems[#menuItems + 1] = {
            header = "Kıyafet Mağazası",
            icon = "fas fa-shirt",
            isMenuHeader = true -- Set to true to make a nonclickable title
        }
        menuItems[#menuItems + 1] = {
            header = header,
            txt = "Giyinmek için geniş bir ürün yelpazesinden seçim yapın.",
            params = {
...
RegisterNetEvent("fivem-appearance:client:deleteOutfitMenu", function(data)
    QBCore.Functions.TriggerCallback('fivem-appearance:server:getOutfits', function(result)
        ...
        for i = 1, #result, 1 do
            outfitMenu[#outfitMenu + 1] = {
                header = 'Silinecek Kıyafet "' .. result[i].outfitname .. '"',
                txt = 'Silmeyi onaylıyor musun?',
                params = {
...
RegisterNetEvent('fivem-appearance:client:reloadSkin', function()
    ...
    if InCooldown() or CheckPlayerMeta() or IsPedInAnyVehicle(playerPed, true) or IsPedFalling(playerPed) then
        QBCore.Functions.Notify("Yenile'yi şu anda kullanamazsınız!", "error")
        return
    end
...
RegisterNetEvent("fivem-appearance:client:ClearStuckProps", function()
    if InCooldown() or CheckPlayerMeta() then
        QBCore.Functions.Notify("Proptemizle'i şu anda kullanamazsınız!", "error")
        return
    end
        ...
        if isPointInside then
            ...
            if jobName == (currentStore.job or currentStore.gang) then
                ...
                if zoneName == 'clothing' then
                    exports['qb-core']:DrawText(prefix .. '[F3] KIYAFETÇI <br>UCRET: $' .. Config.ClothingCost)
                elseif zoneName == 'barber' then
                    exports['qb-core']:DrawText(prefix .. '[F3] BERBER <br>UCRET: $' .. Config.BarberCost)
                elseif zoneName == 'tattoo' then
                    exports['qb-core']:DrawText(prefix .. '[F3] DOVMECI <br>UCRET: $' .. Config.TattooCost)
                elseif zoneName == 'surgeon' then
