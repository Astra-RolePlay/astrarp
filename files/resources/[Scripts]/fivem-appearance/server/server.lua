...
RegisterServerEvent("fivem-appearance:server:chargeCustomer", function(shopType)
    ...
    if Player.Functions.RemoveMoney('cash', money) then
        TriggerClientEvent("QBCore:Notify", src, "$" .. money .. " Ödendi!", "success")
    else
        TriggerClientEvent("QBCore:Notify", src, "Yeterli paran yoktu! Sistemden yararlanmaya çalıştı!", "error")
    end
end)
    ...
    if model and components and props then
        MySQL.Async.insert(
            ...
            {Player.PlayerData.citizenid, name, model, json.encode(components), json.encode(props)}, function(id)
                ...
                TriggerClientEvent('QBCore:Notify', src, 'Kıyafet ' .. name .. ' Adlı Gardroba Eklendi.', 'success')
            end)
    end
end)
...
RegisterNetEvent("fivem-appearance:server:saveManagementOutfit", function(outfitData)
    ...
    MySQL.Async.insert("INSERT INTO management_outfits (job_name, type, minrank, name, gender, model, props, components) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
        ...
        function()
            TriggerClientEvent('QBCore:Notify', src, 'Kıyafet ' .. outfitData.Name .. ' Adlı Gardroba Eklendi.', 'success')
        end)
end)
...
if Config.EnablePedMenu then
    QBCore.Commands.Add('skin', 'Open Ped Menu', {{
        name = 'id',
...
QBCore.Commands.Add('yenile', 'Reloads your character', {}, false, function(source, _)
    TriggerClientEvent("fivem-appearance:client:reloadSkin", source)
end)

QBCore.Commands.Add('proptemizle', 'Removes all the props attached to the entity', {}, false, function(source, _)
