...
local function uuid()
    ...
    local template = 'xxxx-PDCAM-xxx-xx'
...
AddEventHandler('rp_vehicles:server:GoPro:CreateStream', function(veh, dashType, item)
    local src = source
    local Ply = QBCore.Functions.GetPlayer(src)
    local cid = Ply.PlayerData.citizenid
    local networkVehicleEntity = NetworkGetEntityFromNetworkId(veh)
    Ply.Functions.RemoveItem(item.name, 1, item.slot)
	local streamTitle = ""
...
exports("getVehicleDashcams", function()
    return GoProStreams
end)