...
RegisterNetEvent('luckywheel:getwheel', function()
    ...
    if bankBalance >= MinAmount then
        ...
        return TriggerClientEvent('QBCore:Notify', source, "Bankanızda yeterli miktarda bakiye yok.", "error")
    end
end)
...
RegisterNetEvent('luckywheel:give', function(source, price)
	...
	elseif price.type == 'item' then
		...
		TriggerClientEvent('QBCore:Notify', source, "Tebrikler, şunu kazandınız: "..price.count.." "..price.name.."!", 'success')
	...
	elseif price.type == 'money' then
		...
		TriggerClientEvent('QBCore:Notify', source, "Tebrikler, şunu kazandınız: $"..price.count, 'success')

	elseif price.type == 'weapon' then
		TriggerClientEvent("chCasinoWall:bigWin", source)

		Player.Functions.AddItem(price.name, 1, slot, {["quality"] = 100})
		TriggerClientEvent('QBCore:Notify', source, "Tebrikler, bir adet pistol kazandınız!", 'success')
