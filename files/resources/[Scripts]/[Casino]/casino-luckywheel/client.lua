        ...
        if distance < 2.9 and not _isRolling then
            if distance < 2.8 and not _isRolling then
				...
				if Config.LuckyWheelPrompt == 'press' then 
					text = "<b>Gazino Şans Çarkı</b></p>Oturmak için [E] tuşuna basabilirsiniz: $"..Config.startingPrice
				...
				elseif Config.LuckyWheelPrompt == 'peek' then
					text = "<b>Gazino Şans Çarkı</b></p> $"..Config.startingPrice.." spin ücreti"
            ...
            if inZone and not alreadyEnteredZone then
				...
				exports['qb-core']:DrawText('Şansını Dene','left')
            end
...
RegisterNetEvent("luckywheel:client:startWheel", function()
	local hasitem = QBCore.Functions.HasItem('casino_vip', 1)
	if hasitem then
		QBCore.Functions.TriggerCallback('kml-gazino:server:spincheck', function(check)
			local spinsayis = check[1]
			local spinsayi = spinsayis['spin']
			if tonumber(spinsayi) >= 20 then
				QBCore.Functions.Notify('12 saatte sadece 20 kere yapabilirsin', 'error') 
			else
				QBCore.Functions.TriggerCallback('kml-gazino:server:spincalisti', function(kontrol)
					if kontrol then
		...
		QBCore.Functions.Notify('Gazinodan VIP üyeliğiniz yok', 'error', 3500)
	end
end)
...
RegisterNetEvent("dojLuckywheel:winCar", function() 
	...
	QBCore.Functions.SpawnVehicle(Config.VehiclePrize, function(veh)
		...
		QBCore.Functions.Notify("Tebrikler, bir araç kazandınız!", "primary", 3500)
	end, coords, true)            
end)
