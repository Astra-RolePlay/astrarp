	...
	exports["qb-blackjack"]:SetGetChipsCallback(function(source)
		...
		if Chips ~= nil then 
			if Chips.amount >= minAmount then
				...
				return TriggerClientEvent('QBCore:Notify', src, 'Yeterli sayıda siyah jetonun yok', 'error')
			end
		else
			return TriggerClientEvent('QBCore:Notify', src, 'Siyah jetonun yok', 'error')
		end
    end) 
    ...
    exports["qb-blackjack"]:SetGiveChipsCallback(function(source, amount)
        ...
        if Player ~= nil then
			if Player.Functions.AddItem('casino_blackchip', amount, nil, {["quality"] = 100}) then
				...
				TriggerClientEvent('QBCore:Notify', src, "KAZANDIN! "..amount.." siyah jeton eklendi!")
			else
				TriggerClientEvent('QBCore:Notify', src, 'Çok fazla siyah jetonun var', 'error')
			end
        end
    end)
end
