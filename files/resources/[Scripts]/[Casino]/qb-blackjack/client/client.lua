...
RegisterNetEvent("BLACKJACK:RequestBets", function(index)
	...
	QBCore.Functions.TriggerCallback('BLACKJACK:server:blackChipsAmount', function(result)
		...
		CreateThread(function()
			...
			exports['qb-core']:DrawText("<strong>Max Bahis:</strong> Q</p><strong>Bahsi Düzenle: </strong>←/→</p><strong>Bahis Koy: </strong>ENTER</p><strong>ESC:</strong> Çık", 'left')
			while true do
				Wait(0)
				exports['casinoUi']:DrawCasinoUi('show', "Diamond Casino Blackjack</p>Kalan Süre: 0:"..timeLeft.."</p>Mevcut Bahis: "..bet.." </p>Mevcut Jetonlar: "..retval)   
				...
				if IsControlJustPressed(1, 201) then -- ENTER / A
					...
					if canBet then
						...
						QBCore.Functions.Notify('Jetonun yok...', 'error', 3500)
					end
				end
			end
		end)
...
RegisterNetEvent("doj:client:blackjackMenu", function(args)
	...
	elseif args == 3 then
		...
		if canBet then
			...
			QBCore.Functions.Notify("İkiye katlamak için yeterli sayıda siyah jetonun yok.", "error")
			exports['casinoUi']:DrawCasinoUi('show', "Diamond Casino Blackjack</p>Rakip: "..dealerValue[g_seat].."</p>Eli: "..handValue(hand))  
		...
		if canBet then
			...
			QBCore.Functions.Notify("Ayırabileceğin yeterli sayıda jeton yok.", "error")
			exports['casinoUi']:DrawCasinoUi('show', "Diamond Casino Blackjack</p>Rakip: "..dealerValue[g_seat].."</p>El: "..handValue(hand))  
			TriggerEvent("casino:context:hit&stand")
		end
	end 
end)
...
RegisterNetEvent("BLACKJACK:RequestMove", function()
	...
	exports['qb-core']:DrawText('Bahisler 30 saniye içinde kapanıyor', 'left')
	if leavingBlackjack == true then 
		leaveBlackjack() 
		return 
	elseif  #hand < 3 and #splitHand == 0 then
		TriggerEvent("casino:context:hit&doubledown")
		exports['casinoUi']:DrawCasinoUi('show', "Diamond Casino Blackjack</p>Rakip: "..dealerValue[g_seat].."</p>Elin: "..handValue(hand))  
	elseif CanSplitHand(hand) == true then
		TriggerEvent("casino:context:hit&split")
		exports['casinoUi']:DrawCasinoUi('show', "Diamond Casino Blackjack</p>Rakip: "..dealerValue[g_seat].."</p>Elin: "..handValue(hand).."</p>[Eli Böl: "..handValue(splitHand).."]") 
				...
				if #(GetEntityCoords(PlayerPedId()) - vector3(cord.x, cord.y, cord.z)) < 3.0 then
					...
					if GetEntityCoords(tableObj) ~= vector3(0.0, 0.0, 0.0) then
						...
						if #(pCoords - coords) < 1.5 and not IsSeatOccupied(coords, 0.5) and canSit then
							...
							if highStakes then
								text = "<b>Diamond Casino Blackjack (Yüksek-Limit)</b></p>[E] Otur"
							else
								text = "<b>Diamond Casino Blackjack</b></p>[E] Otur"
							end
