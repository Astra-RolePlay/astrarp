            ...
            CreateThread(function()
                while selectedRulett ~= nil do
                    ...
                    if IsDisabledControlPressed(0, 172) then
                        ...
                        QBCore.Functions.Notify('+'..currentBetAmount.." bahis [arttırıldı]",'success')
                    ...
                    elseif IsDisabledControlPressed(0, 173) then
                        if currentBetAmount > 0 then
                            ...
                            changeBetAmount(currentBetAmount)
                            QBCore.Functions.Notify('-'..currentBetAmount.." bahis [azaltıldı]",'primary')
                ...
                while selectedRulett ~= nil do
                    ...
                    if Config.allowCustomBet then
                        if IsDisabledControlJustPressed(0, 22) then --Custom Bet [space]
                            local tmpInput = getGenericTextInput('Ne kadar jeton bahse girmek istersin?')
                            if tonumber(tmpInput) then
                                tmpInput = tonumber(tmpInput)
                                if tmpInput > 0 then
                                    changeBetAmount(tmpInput)
                                    QBCore.Functions.Notify('Mevcut Bahis: '..currentBetAmount..' jeton','success')
                                end
                            end
                        end
                    else
                                        ...
                                        if currentBetAmount > 0 then
                                            if Config.RulettTables[selectedRulett] ~= nil then
                                                if currentBetAmount >= Config.RulettTables[selectedRulett].minBet and currentBetAmount <= Config.RulettTables[selectedRulett].maxBet then
                                                    ...
                                                    QBCore.Functions.Notify('Bahsiniz bu masa için çok düşük ya da çok yüksek.','error')

                                                end
                                            end
                                        else
                                            QBCore.Functions.Notify('Bahsinizi arttırmanız gerekli','error')
                                        end
                                    end
                                end
                            end
        ...
        if closetoRulett and selectedRulett == nil then
            for k, v in pairs(Rulettek) do
                if DoesEntityExist(v.tableObject) then
                    ...
                    if dist < 2.4 then
                        if dist < 2.3 then
                            -- exports['textUi']:DrawTextUi('show',"Diamond Casino Roulette</p>[E] to sit down")
                            exports['qb-core']:DrawText('[E] Otur', 'left')
                            ...
                            if IsControlJustPressed(0, 38) then
                                QBCore.Functions.TriggerCallback('QBCore:HasItem', function(HasItem)
                                    if HasItem then
							            ...
							            QBCore.Functions.Notify('Gazinonun üyesi değilsin', 'error', 3500)
						            end
					            end, 'casino_member')
                            end
                            break
...
AddEventHandler('client_callback:rulett:taskSitDown',
    function(rulettIndex, chairData)
        ...
        QBCore.Functions.Notify("Oturuluyor...", "primary", 3200)
...
function casinoNuiUpdateGame(rulettIndex, ido, statusz)
    QBCore.Functions.TriggerCallback('BLACKJACK:server:blueChipsAmount', function(result)
        ...
        if selectedRulett == rulettIndex then
            if not statusz then
                exports['casinoUi']:DrawCasinoUi('show', "Diamond Casino Blackjack</p>"..ido.." saniye kaldı</p>Mevcut Bahis: "..currentBetAmount.." jeton</p>Mevcut Jeton: "..retval)
                if Config.allowCustomBet then
                    -- exports['textUi']:DrawTextUi('show', "Bahsini Düzenle: <strong>↑/↓</strong></p>LEFT CLICK: Bet number</p>SPACEBAR: Custom Amount</p>E: Change camera</p>ESC: Exit")
                    exports['qb-core']:DrawText("Bahsini Düzenle: <strong>↑/↓</strong></p>SOL TIK: Bahis sayısı</p>SPACE: Miktarı Özelleştir</p>E: Kamerayı Değiştir</p>ESC: Çık", 'left')
                else
                    -- exports['textUi']:DrawTextUi('show', "Bahsini Düzenle: <strong>↑/↓</strong></p>LEFT CLICK: Bet number</p>E: Change camera</p>ESC: Exit")
                    exports['qb-core']:DrawText("Bahsini Düzenle: <strong>↑/↓</strong></p>SOL TIK: Bahis sayısı</p>E: Kamerayı Değiştir</p>ESC: Çık", 'left')
                end
            else
                -- exports['textUi']:DrawTextUi('show', "The game is starting..") 
                exports['qb-core']:DrawText('Oyun başlıyor..', 'left')
                hideUi()
            end
        end
    end)
