...
function giveChips(source, amount)
    ...
    if Player ~= nil then
        if Player.Functions.AddItem('casino_bluechip', amount, nil, {["quality"] = 100}) then
            ...
            TriggerClientEvent('QBCore:Notify', src, amount.." mavi jeton kazandın!")
        else
            TriggerClientEvent('QBCore:Notify', src, 'Çok fazla mavi jetonun var', 'error')
        end 
    end
end 
...
RegisterNetEvent('server_remote:rulett:taskSitDown',function(rulettIndex, chairData)
    ...
    if aktivRulettek[rulettIndex] ~= nil then
        if aktivRulettek[rulettIndex].chairsUsed[chairId] ~= nil then
            return TriggerClientEvent('QBCore:Notify', source, 'Bu sandalye dolu.','error')
        else
...
RegisterNetEvent('casino:taskStartRoulette',function(rulettIndex, chairId)
    ...
    if aktivRulettek[rulettIndex].chairsUsed[chairId] == nil then
        ...
        TriggerClientEvent('QBCore:Notify', source, 'Bu sandalye dolu.','error')
    end
end)
...
function CheckWinners(bets, WinningBetIndex)
    ...
    for targetSrc, _ in pairs(playersLoss) do
        ...
        if chairId ~= nil then
            ...
            TriggerClientEvent('QBCore:Notify', targetSrc, 'KAYBETTİN! Bir dahakine iyi şanslar', "error")
        end
    end
...
RegisterNetEvent('casino:taskBetRulett',function(rulettIndex, betId, betAmount)
    ...
    if aktivRulettek[rulettIndex] ~= nil then
        if aktivRulettek[rulettIndex].statusz then
            return TriggerClientEvent('QBCore:Notify', src, 'Oyun başladı, şu anda bahis koyamazsınız', "error")
        end
        local chipsAmount = getPlayerChips(src)
        if chipsAmount ~= nil then
            if chipsAmount >= betAmount then 
                removeChips(src, betAmount)
                TriggerClientEvent('QBCore:Notify', src, betAmount..' jeton bahse katıldı ['..betId..']')
                ...
                TriggerClientEvent('QBCore:Notify', src, 'Yeterli sayıda mavi jetonun yok.', "error")
            end
        else
            TriggerClientEvent('QBCore:Notify', src, 'Mavi jetonun yok', 'error')
        end
    else 
