			...
			RenderScriptCams(true, 900, 900, true, false)
				...
				CreateThread(function()
					while selectedSlot ~= nil do
						...
						exports['qb-core']:DrawText("Ayağa Kalk: Backspace</p>Döndür: Enter/Space</p>Kamerayı Değiştir: E", 'left')
						QBCore.Functions.TriggerCallback('doj:server:whiteChipsAmount', function(result)
							exports['casinoUi']:DrawCasinoUi('show', "Diamond Casino Slot Makinesi</p>Mevcut Jeton: "..result)   
						end)
						Wait(0)
	...
	while true do
		...
		for k, v in pairs(Slots) do
			...
			if dist < 1.8 then
				...
				wait = 5
				inZone  = true
				text = '<b>Diamond Casino Slot</b> </p>[E] Otur</p>'..v.data.SlotName..'</p>'..v.data.bet..' beyaz jeton'
				if IsControlJustPressed(0, 38) then
					QBCore.Functions.TriggerCallback('QBCore:HasItem', function(HasItem)
						if HasItem then
							QBCore.Functions.TriggerCallback('casino:slots:isSeatUsed',function(used)
								if used then
									QBCore.Functions.Notify('Oturma yeri dolu') 
								else
									ACTIVE_SLOT = v.index
							...
							QBCore.Functions.Notify('Gazinonun bir üyesi değilsiniz', 'error', 3500)
						end
					end, "casino_member")
				end
			end
