...
AddEventHandler('casino:taskStartSlots',function(index, data)
	...
	if Chips ~= nil then 
		if Chips.amount >= minAmount then
			...
			return TriggerClientEvent('QBCore:Notify', src, 'Yeterli sayıda beyaz jetonun yok', 'error')
		end
	else
		return TriggerClientEvent('QBCore:Notify', src, 'Beyaz jetonun yok', 'error')
	end
end) 
...
function CheckForWin(w, data)
	...
	if total > 0 then
		...
		if Player.Functions.AddItem('casino_whitechip', total, nil, {["quality"] = 100}) then
			...
			TriggerClientEvent('QBCore:Notify', src, "KAZANDIN! "..total.." beyaz jeton eklendi!")
		else
			TriggerClientEvent('QBCore:Notify', src, 'Çok fazla beyaz jetonun var', 'error')
		end

	else
		TriggerClientEvent('QBCore:Notify', src, "Kaybettin, bir dahakine iyi şanslar!", 'error')
	end
end
