...
CONTROLS	
	...
	Number 1 Key: TA LEFT Patern (Passenger To Driver)
    Number 2 Key: TA CENTER Patern (In to Out)
    Number 3 Key: TA LEFT Patern (Driver To Passenger)
---------------------------------------------------
]]
						...
						if not IsPauseMenuActive() then
							...
							if IsDisabledControlJustReleased(0, 85) or IsDisabledControlJustReleased(0, 246) then
								if IsVehicleSirenOn(veh) then
									TriggerEvent("lux_vehcontrol:ELSClick", "Off", 0.5) -- Off
									SetVehicleSiren(veh, false)
								else
									TriggerEvent("lux_vehcontrol:ELSClick", "On", 0.6) -- On
									Citizen.Wait(100)
							...
							elseif IsControlJustPressed(1, 158) or IsDisabledControlJustPressed(1, 158) then
								if IsVehicleExtraTurnedOn(GetVehiclePedIsUsing(GetPlayerPed(-1), false), 10) then
								else
									...
									SetVehicleExtra(GetVehiclePedIsUsing(GetPlayerPed(-1), false), 10, false)
									TriggerEvent("lux_vehcontrol:ELSClick", "On", 0.5)
								end
					
							-- First thing
							elseif IsControlJustPressed(1, 157) or IsDisabledControlJustPressed(1, 157) then
								if IsVehicleExtraTurnedOn(GetVehiclePedIsUsing(GetPlayerPed(-1), false), 11) then
								else
										SetVehicleExtra(GetVehiclePedIsUsing(GetPlayerPed(-1), false), 11, false)
										SetVehicleExtra(GetVehiclePedIsUsing(GetPlayerPed(-1), false), 12, true)
							...
							-- First thing
							elseif IsControlJustPressed(1, 160) or IsDisabledControlJustPressed(1, 160) then
								if IsVehicleExtraTurnedOn(GetVehiclePedIsUsing(GetPlayerPed(-1), false), 12) then
								else
									SetVehicleExtra(GetVehiclePedIsUsing(GetPlayerPed(-1), false), 12, false)
									SetVehicleExtra(GetVehiclePedIsUsing(GetPlayerPed(-1), false), 10, true)
									SetVehicleExtra(GetVehiclePedIsUsing(GetPlayerPed(-1), false), 11, true)
									TriggerEvent("lux_vehcontrol:ELSClick", "On", 0.5)
								end	
