    ...
    if client then
        ...
        while status do
            ...
            if distance < 1.0 and client then
                QBCore.Functions.DrawText3D(Config.DeliveryCoords[random]["x"], Config.DeliveryCoords[random]["y"],
                    Config.DeliveryCoords[random]["z"], Lang:t("qbmenu.deliver"))
...
function PacketSell2()
    ...
    QBCore.Functions.TriggerCallback('6x_itcompjob:itemcheck', function(data)
        if data then
            QBCore.Functions.Progressbar("packetsell", Lang:t("progress.packetselling"), Config.ProgressbarTime, false,
                true, {
