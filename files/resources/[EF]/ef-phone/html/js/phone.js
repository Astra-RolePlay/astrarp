...
$(document).on("click", ".phone-app-footer-button", function (e) {
  ...
  if (PressedFooterTab !== CurrentFooterTab) {
    var PreviousTab = $(this)
      .parent()
      .find('[data-phonefootertab="' + CurrentFooterTab + '"');

    $(".phone-app-footer")
      .find('[data-phonefootertab="' + CurrentFooterTab + '"')
      .removeClass("phone-selected-footer-tab");
...
QB.Phone.Functions.SetupRecentCalls = function (recentcalls) {
  ...
  $.each(recentcalls, function (i, recentCall) {
    ...
    var elem =
      '<div class="phone-recent-call" id="recent-' +
      i +
      '"><div class="phone-recent-call-image">' +
      FirstLetter +
      '</div> <div class="phone-recent-call-name">' +
      recentCall.name +
      '</div> <div class="phone-recent-call-type"><i class="' +
      TypeIcon +
      '" style="' +
      IconStyle +
      '"></i></div> <div class="phone-recent-call-time">' +
      recentCall.time +
      "</div> </div>";
...
QB.Phone.Functions.LoadContacts = function (myContacts) {
  ...
  if (myContacts !== null) {
    $.each(myContacts, function (i, contact) {
      ...
      if (contact.name == "")
        contact.name = "Hmm, I shouldn't be able to do this...";
      var ContactElement =
        '<div class="phone-contact" data-contactid="' +
        i +
        '"><div class="phone-contact-firstletter" style="background-color: #e74c3c;">' +
        contact.name.charAt(0).toUpperCase() +
        '</div><div class="phone-contact-name">' +
        contact.name +
        '</div><div class="phone-contact-actions"><i class="fas fa-sort-down"></i></div><div class="phone-contact-action-buttons"> <i class="fas fa-phone-volume" id="phone-start-call"></i> <i class="fab fa-whatsapp" id="new-chat-phone" style="font-size: 2.5vh;"></i> <i class="fas fa-user-edit" id="edit-contact"></i> </div></div>';
      if (contact.status) {
        ContactElement =
          '<div class="phone-contact" data-contactid="' +
          i +
          '"><div class="phone-contact-firstletter" style="background-color: #2ecc71;">' +
          contact.name.charAt(0).toUpperCase() +
          '</div><div class="phone-contact-name">' +
          contact.name +
          '</div><div class="phone-contact-actions"><i class="fas fa-sort-down"></i></div><div class="phone-contact-action-buttons"> <i class="fas fa-phone-volume" id="phone-start-call"></i> <i class="fab fa-whatsapp" id="new-chat-phone" style="font-size: 2.5vh;"></i> <i class="fas fa-user-edit" id="edit-contact"></i> </div></div>';
...
$(document).on("click", "#new-chat-phone", function (e) {
  ...
  var ContactData = $("[data-contactid='" + ContactId + "']").data(
    "contactData"
  );

  if (ContactData.number !== QB.Phone.Data.PlayerData.charinfo.phone) {
    $.post(
      "https://ef-phone/GetWhatsappChats",
      JSON.stringify({}),
      function (chats) {
    ...
    setTimeout(function () {
      ...
      $.post(
        "https://ef-phone/GetWhatsappChat",
        JSON.stringify({ phone: ContactData.number }),
        function (chat) {
...
$(document).on("click", "#edit-contact", function (e) {
  ...
  if (CurrentEditContactData.name == "")
    CurrentEditContactData.name = "Hmm, I shouldn't be able to do this...";
...
$(document).on("click", ".phone-contact-actions", function (e) {
  ...
  if (OpenedContact === null) {
    ...
    $(FocussedContact)
      .find(".phone-contact-action-buttons")
    ...
    var PreviousContact = $(".phone-contact-list").find(
      '[data-contactid="' + OpenedContact + '"]'
    );
    $(PreviousContact)
      .find(".phone-contact-action-buttons")
      .fadeOut(100, function () {
...
$(document).on("click", "#phone-airdrop", function (e) {
  e.preventDefault();
  $.post("https://ef-phone/GiveContactDetails");
});
...
$(document).on("click", "#add-contact-save", function (e) {
  ...
  if (ContactName != "" && ContactNumber != "") {
    ...
    if (SelectedSuggestion !== null) {
      $.post(
        "https://ef-phone/RemoveSuggestion",
        JSON.stringify({
  ...
  if (!Canceled) {
    if (!QB.Phone.Data.CallActive) {
      ...
      setTimeout(function () {
        ...
        $(".phone-app").css({ display: "none" });
        QB.Phone.Functions.HeaderTextColor("white", 400);
        $("." + QB.Phone.Data.currentApplication + "-app").css({
          display: "none",
        });
...
QB.Phone.Functions.SetupSuggestedContacts = function (Suggested) {
  ...
  if (AmountOfSuggestions > 0) {
    ...
    $.each(Suggested, function (index, suggest) {
      var elem =
        '<div class="suggested-contact" id="suggest-' +
        index +
        '"> <i class="fas fa-exclamation-circle"></i> <span class="suggested-name">' +
        suggest.name[0] +
        " " +
        suggest.name[1] +
        ' &middot; <span class="suggested-number">' +
        suggest.number +
        "</span></span> </div>";
...
$(document).on("click", ".suggested-contact", function (e) {
  ...
  $(".phone-add-contact-name").val(
    SuggestionData.name[0] + " " + SuggestionData.name[1]
  );
