...
local pinkcagecoord = vector3(323.5, -217.26, 54.09)
local pinkcage = {
...
AddEventHandler('playerSpawned', function()
    if firstSpawn then
        ...
        QBCore.Functions.Notify("Yeni Motel Odan Verildi! Oda No: " ..currentmotel, 'success', 5000)
        firstSpawn = false
    end
end)

RegisterCommand('yenioda', function()
    currentmotel = math.random(1, #pinkcage)
    QBCore.Functions.Notify("Yeni Motel Odan Verildi! Oda No: " ..currentmotel, 'success', 5000)
end)
RegisterCommand('motelcik', function()
    ...
    if moteldistance <= 45.0 then
        SetEntityCoords(player, 335.03, -227.36, 54.22, 0, 0, 0, 0) 
        SetEntityHeading(player, 346.79)
            ...
            for j = 1, #pinkcage, 1 do
                ...
                if doordistance <= 2.0 then
                    if pinkcage[j].locked and currentmotel == j then 
                        ...
                        exports['qb-core']:DrawText("[Oda: "..j.."] - [E] - Kilitli")
                    elseif not pinkcage[j].locked and currentmotel == j then 
                        
                        exports['qb-core']:DrawText("[Oda: "..j.."] - [E] - Kilitli Değil")

                    elseif pinkcage[j].locked and currentmotel ~= j then
                      
                        exports['qb-core']:DrawText("[Oda: "..j.."] - Kapalı")

                    elseif not pinkcage[j].locked and currentmotel ~= j then
                        
                        exports['qb-core']:DrawText("[Oda: "..j.."] - Açık")
                    end
            ...
            if moteldistance <= 45.0 then
                if doordistance <= 30.0 then
                    ...
                    if stashdistance <= 1.5 then
                        ...
                        exports['qb-core']:DrawText('[E] Depo')
                    ...
                    if clothedistance <= 1.5 then
                        ...
                        exports['qb-core']:DrawText('[E] Kıyafet ')
