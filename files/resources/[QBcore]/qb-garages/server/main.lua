...
QBCore.Functions.CreateCallback("qb-garage:server:GetGarageVehicles", function(source, cb, garage, garageType, category)
    ...
    if garageType == "public" then --Public garages give player cars in the garage only
        MySQL.query('SELECT * FROM player_vehicles WHERE citizenid = ? AND state = ?', { pData.PlayerData.citizenid, 1 },
            function(result)
                local vehs = {}
                if result[1] then
    ...
    elseif garageType == "depot" then --Depot give player cars that are not in garage only
        MySQL.query('SELECT * FROM player_vehicles WHERE citizenid = ? AND (state = ? OR garage = ? OR garage IS NULL)',
            { pData.PlayerData.citizenid, 0, garage }, function(result)
            local tosend = {}
            if result[1] then
...
RegisterNetEvent('qb-garage:server:updateVehicle',
    function(state, fuel, engine, body, plate, properties, garage, location, damage)
        if location and type(location) == 'vector3' then
            if StoreDamageAccuratly then
                ...
                'UPDATE player_vehicles SET state = ?, garage = ?, fuel = ?, engine = ?, body = ?, mods = ?, parkingspot = ?, damage = ? WHERE plate = ?',
                    { state, garage, fuel, engine, body, json.encode(properties), json.encode(location), json.encode(
                    damage), plate })
            else
                MySQL.update(
