...
local function ParkOwnedVehicle(veh, garageName, vehLocation, plate)
    ...
    if FuelScript then
        ...
        totalFuel = exports['ed-fuel']:GetFuel(veh) -- Don't change this. Change it in the  Defaults to legacy fuel if not set in the config
    end
...
local function UpdateVehicleSpawnerSpawnedVehicle(veh, garage, heading, cb)
    ...
    if FuelScript then
        ...
        exports['ed-fuel']:SetFuel(veh, 100) -- Don't change this. Change it in the  Defaults to legacy fuel if not set in the config
    end
...
function UpdateSpawnedVehicle(spawnedVehicle, vehicleInfo, heading, garage, properties)
    ...
    if garage.useVehicleSpawner then
        ...
        if FuelScript then
            ...
            exports['ed-fuel']:SetFuel(spawnedVehicle, 100) -- Don't change this. Change it in the  Defaults to legacy fuel if not set in the config
        end
        TriggerServerEvent("qb-garage:server:UpdateSpawnedVehicle", plate, true)
    else
        if plate then
        ...
        if FuelScript then
            ...
            exports['ed-fuel']:SetFuel(spawnedVehicle, vehicleInfo.fuel) -- Don't change this. Change it in the  Defaults to legacy fuel if not set in the config
        end
    ...
    TriggerServerEvent('vehiclekeys:server:SetVehicleOwner', vehicleInfo.plate)
...
RegisterNetEvent('qb-garages:client:TakeOutDepot', function(data)
    ...
    local vehHandle = OutsideVehicles[vehicle.plate]
    
    if vehHandle then
        DeleteEntity(vehHandle)
        OutsideVehicles[vehicle.plate] = nil
        QBCore.Functions.Notify("Aracınız Çekilmişten 60$ Karşılığında Getirildi.", "success", 5000)
    end
