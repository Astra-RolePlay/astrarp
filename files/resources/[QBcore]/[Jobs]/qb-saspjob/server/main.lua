...
QBCore.Commands.Add('fine', Lang:t("commands.fine"), {{name = 'id', help = Lang:t('info.player_id')}, {name = 'amount', help = Lang:t('info.amount')}}, false, function(source, args)
    ...
    if biller.PlayerData.job.name == "sasp" then
        if billed ~= nil then
            if biller.PlayerData.citizenid ~= billed.PlayerData.citizenid then
                if amount and amount > 0 then
                    ...
                    exports['qb-management']:AddMoney('sasp', amount)
                else
...
QBCore.Commands.Add('911p', Lang:t("commands.police_report"), {{name='message', help= Lang:t("commands.message_sent")}}, false, function(source, args)
    ...
    for _, v in pairs(players) do
        if v and v.PlayerData.job.name == 'sasp' and v.PlayerData.job.onduty then
            ...
            TriggerClientEvent('sasp:client:policeAlert', v.PlayerData.source, coords, message)
        end
    end
end)
...
QBCore.Functions.CreateCallback('sasp:server:IsPoliceForcePresent', function(_, cb)
    local retval = false
    local players = QBCore.Functions.GetQBPlayers()
    for _, v in pairs(players) do
...
RegisterNetEvent('sasp:server:policeAlert', function(text)
    local src = source
    local ped = GetPlayerPed(src)
    local coords = GetEntityCoords(ped)
    local players = QBCore.Functions.GetQBPlayers()
    for _, v in pairs(players) do
        if v and v.PlayerData.job.name == 'sasp' and v.PlayerData.job.onduty then
            ...
            TriggerClientEvent("qb-phone:client:addPoliceAlert", v.PlayerData.source, alertData)
            TriggerClientEvent('sasp:client:policeAlert', v.PlayerData.source, coords, text)
        end
    end
end)
...
RegisterNetEvent('sasp:server:CuffPlayer', function(playerId, isSoftcuff)
    ...
    if not Player or not CuffedPlayer or (not Player.Functions.GetItemByName("pkelepce") and Player.PlayerData.job.name ~= "sasp") then return end
...
RegisterNetEvent('sasp:server:EscortPlayer', function(playerId)
    ...
    if (Player.PlayerData.job.name == "sasp" or Player.PlayerData.job.name == "ambulance") or (EscortPlayer.PlayerData.metadata["ishandcuffed"] or EscortPlayer.PlayerData.metadata["isdead"] or EscortPlayer.PlayerData.metadata["inlaststand"]) then
...
RegisterServerEvent("qb-saspjob:server:Charge", function(citizen, price)
    local biller = QBCore.Functions.GetPlayer(source)
