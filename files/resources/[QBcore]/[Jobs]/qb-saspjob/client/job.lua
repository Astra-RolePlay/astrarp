...
function TakeOutImpound(vehicle)
    ...
    if coords then
		...
		SetVehicleDirtLevel(veh, 0.0)
                ...
                TriggerServerEvent('sasp:server:TakeOutImpound', vehicle.plate, currentGarage)
                closeMenuFull()
...
local function IsArmoryWhitelist() -- being removed
    ...
    if QBCore.Functions.GetPlayerData().job.name == 'sasp' then
        retval = true
    end
    return retval
end
...
function MenuGarage(currentSelection)
    ...
    for veh, label in pairs(authorizedVehicles) do
        vehicleMenu[#vehicleMenu+1] = {
            ...
            txt = "",
            params = {
                event = "sasp:client:TakeOutVehicle",
                args = {
                    vehicle = veh,
...
function MenuImpound(currentSelection)
    ...
    QBCore.Functions.TriggerCallback("sasp:GetImpoundedVehicles", function(result)
        ...
        if result == nil then
            ...
            for _ , v in pairs(result) do
                ...
                impoundMenu[#impoundMenu+1] = {
                    ...
                    params = {
                        event = "sasp:client:TakeOutImpound",
                        args = {
                            vehicle = v,
                            currentSelection = currentSelection
                        }
...
--Events
RegisterNetEvent('sasp:client:showFingerprint', function(playerId)
    openFingerprintUI()
    FingerPrintSessionId = playerId
end)

RegisterNetEvent('sasp:client:showFingerprintId', function(fid)
    SendNUIMessage({
...
RegisterNUICallback('doFingerScan', function(_, cb)
    TriggerServerEvent('sasp:server:showFingerprintId', FingerPrintSessionId)
    cb("ok")
end)

RegisterNetEvent('sasp:client:SendEmergencyMessage', function(coords, message)
    TriggerServerEvent("sasp:server:SendEmergencyMessage", coords, message)
    TriggerEvent("sasp:client:CallAnim")
end)

RegisterNetEvent('sasp:client:EmergencySound', function()
    PlaySound(-1, "Event_Start_Text", "GTAO_FM_Events_Soundset", 0, 0, 1)
end)

RegisterNetEvent('sasp:client:CallAnim', function()
...
RegisterNetEvent('sasp:client:ImpoundVehicle', function(fullImpound, price)
    ...
    if vehicle ~= 0 and vehicle then
        ...
        if #(pos - vehpos) < 5.0 and not IsPedInAnyVehicle(ped) then
           QBCore.Functions.Progressbar('impound', Lang:t('progressbar.impound'), 5000, false, true, {
                ...
                TriggerServerEvent("sasp:server:Impound", plate, fullImpound, price, bodyDamage, engineDamage, totalFuel)
...
RegisterNetEvent('sasp:client:CheckStatus', function()
    ...
    QBCore.Functions.GetPlayerData(function(PlayerData)
        if PlayerData.job.name == "sasp" then
            ...
            if player ~= -1 and distance < 5.0 then
                ...
                QBCore.Functions.TriggerCallback('sasp:GetPlayerStatus', function(result)
                    if result then
                        for _, v in pairs(result) do
                            statusOn = true
                            if statusOn then 
...
RegisterNetEvent("sasp:client:VehicleMenuHeader", function (data)
    MenuGarage(data.currentSelection)
    currentGarage = data.currentSelection
end)


RegisterNetEvent("sasp:client:ImpoundMenuHeader", function (data)
    MenuImpound(data.currentSelection)
    currentGarage = data.currentSelection
end)

RegisterNetEvent('sasp:client:TakeOutImpound', function(data)
    if inImpound then
...
RegisterNetEvent('sasp:client:TakeOutVehicle', function(data)
    if inGarage then
        local vehicle = data.vehicle
...
RegisterNetEvent('sasp:client:EvidenceStashDrawer', function(data)
...
RegisterNetEvent('qb-saspjob:ToggleDuty', function()
    TriggerServerEvent("QBCore:ToggleDuty")
    TriggerServerEvent("sasp:server:UpdateCurrentCops")
end)

RegisterNetEvent('qb-sasp:client:scanFingerPrint', function()
    local player, distance = GetClosestPlayer()
    if player ~= -1 and distance < 2.5 then
        local playerId = GetPlayerServerId(player)
        TriggerServerEvent("sasp:server:showFingerprint", playerId)
    else
        QBCore.Functions.Notify(Lang:t("error.none_nearby"), "error")
    end
end)
...
RegisterNetEvent('qb-sasp:client:openArmoury', function()
    ...
    SetWeaponSeries()
    TriggerServerEvent("inventory:server:OpenInventory", "shop", "sasp", authorizedItems)
end)

RegisterNetEvent('qb-sasp:client:spawnHelicopter', function(k)
...
RegisterNetEvent("qb-sasp:client:openStash", function()
    TriggerServerEvent("inventory:server:OpenInventory", "stash", "State_"..QBCore.Functions.GetPlayerData().citizenid)
    TriggerEvent("inventory:client:SetCurrentStash", "State_"..QBCore.Functions.GetPlayerData().citizenid)
end)

RegisterNetEvent('qb-sasp:client:openTrash', function()
...
local function stash()
    CreateThread(function()
        while true do
            Wait(0)
            if inStash and PlayerJob.name == "sasp" then
...
local function trash()
    CreateThread(function()
        while true do
            Wait(0)
            if inTrash and PlayerJob.name == "sasp" then
                if PlayerJob.onduty then sleep = 5 end
                if IsControlJustReleased(0, 38) then
                    TriggerServerEvent("inventory:server:OpenInventory", "stash", "Çöp Kutusu", {
                        maxweight = 4000000,
...
local function fingerprint()
    CreateThread(function()
        while true do
            Wait(0)
            if inFingerprint and PlayerJob.name == "sasp" then
                if PlayerJob.onduty then sleep = 5 end
                if IsControlJustReleased(0, 38) then
                    TriggerEvent("qb-sasp:client:scanFingerPrint")
                    break
                end
            else
                break
...
local function armoury()
    CreateThread(function()
        while true do
            Wait(0)
            if inArmoury and PlayerJob.name == "sasp" then
                if PlayerJob.onduty then sleep = 5 end
                if IsControlJustReleased(0, 38) then
                    TriggerEvent("qb-sasp:client:openArmoury")
                    break
                end
            else
                break
...
local function heli()
    CreateThread(function()
        while true do
            Wait(0)
            if inHelicopter and PlayerJob.name == "sasp" then
                if PlayerJob.onduty then sleep = 5 end
                if IsControlJustReleased(0, 38) then
                    TriggerEvent("qb-sasp:client:spawnHelicopter")
                    break
                end
            else
                break
...
local function impound()
    CreateThread(function()
        while true do
            Wait(0)
            if inImpound and PlayerJob.name == "sasp" then
                if PlayerJob.onduty then sleep = 5 end
                if IsPedInAnyVehicle(PlayerPedId(), false) then
                    if IsControlJustReleased(0, 38) then
                        QBCore.Functions.DeleteVehicle(GetVehiclePedIsIn(PlayerPedId()))
...
local function garage()
    CreateThread(function()
        while true do
            Wait(0)
            if inGarage and PlayerJob.name == "sasp" then
                if PlayerJob.onduty then sleep = 5 end
                if IsPedInAnyVehicle(PlayerPedId(), false) then
                    if IsControlJustReleased(0, 38) then
                        QBCore.Functions.DeleteVehicle(GetVehiclePedIsIn(PlayerPedId()))
...
if Config.UseTarget then
        ...
        for k, v in pairs(Config.Locations["stash"]) do
            exports['qb-target']:AddBoxZone("SheriffStash_"..k, vector3(v.x, v.y, v.z), 1.5, 1.5, {
                ...
                options = {
                    {
                        type = "client",
                        event = "qb-sasp:client:openStash",
                        icon = "fas fa-lock",
                        label = "Kişisel Depo",
                        job = "sasp",
                    },
        ...
        for k, v in pairs(Config.Locations["trash"]) do
            exports['qb-target']:AddBoxZone("SheriffTrash_"..k, vector3(v.x, v.y, v.z), 1, 1.75, {
                ...
                options = {
                    {
                        type = "client",
                        event = "qb-sasp:client:openTrash",
                        icon = "fas fa-trash",
                        label = "Çöp Kutusu",
                        job = "sasp",
                    },
        ...
        for k, v in pairs(Config.Locations["fingerprint"]) do
            exports['qb-target']:AddBoxZone("PoliceFingerprint_"..k, vector3(v.x, v.y, v.z), 2, 1, {
                ...
                options = {
                    {
                        type = "client",
                        event = "qb-sasp:client:scanFingerPrint",
        ...
        for k, v in pairs(Config.Locations["armory"]) do
            exports['qb-target']:AddBoxZone("PoliceArmory_"..k, vector3(v.x, v.y, v.z), 5, 1, {
                ...
                options = {
                    {
                        type = "client",
                        event = "qb-sasp:client:openArmoury",
                        icon = "fas fa-archive",
                        label = "Cephanelik",
                        job = "sasp",
                    },
    ...
    stashCombo:onPlayerInOut(function(isPointInside, _, _)
        if isPointInside then
            inStash = true
            if PlayerJob.name == 'sasp' and PlayerJob.onduty then
                ...
                exports['qb-menu']:showHeader({
                    {
                        header = Lang:t('info.evidence_stash', {value = currentEvidence}),
                        params = {
                            event = 'sasp:client:EvidenceStashDrawer',
                            args = {
                    ...
                    exports['qb-menu']:showHeader({
                        {
                            ...
                            params = {
                                event = 'sasp:client:ImpoundMenuHeader',
                                args = {
                                    currentSelection = currentSelection,
                                }
                            }
