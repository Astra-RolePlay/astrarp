...
CreateThread(function(statusId) -- Gunpowder Status when shooting
    while true do
        Wait(1)
        local ped = PlayerPedId()
        if IsPedShooting(ped) then
            ...
            if not WhitelistedWeapon(weapon) then
                shotAmount = shotAmount + 1
                if shotAmount > 1 and CurrentStatusList['gunpowder'] == nil then
                    print("Powder ON")
                    if math.random(1, 10) <= 7 then
                        TriggerEvent('evidence:client:SetStatus', 'gunpowder', 200)
                        print("Powder SET")
...
RegisterCommand('baruttemizle', function()
    TriggerEvent('qb-policejob:baruttemizle')
end)

RegisterNetEvent('qb-policejob:baruttemizle', function()
        if powder then
            local ped = PlayerPedId()
            if IsPedSwimming(ped) then
                powder = false
                QBCore.Functions.Progressbar("baruttemizle", "Barut izleri temizleniyor", 10000, false, true, {
                    ...
                    StopAnimTask(PlayerPedId(), "mp_safehouseshower@male@", "male_shower_idle_b", 1.0)
                    TriggerEvent('evidence:client:SetStatus', 'gunpowder', 0)
                    QBCore.Functions.Notify("Barut izleri temizlendi.", "success")
                end, function() -- Cancel
                    StopAnimTask(PlayerPedId(), "mp_safehouseshower@male@", "male_shower_idle_b", 1.0)
                    TriggerEvent('evidence:client:SetStatus', 'gunpowder', 200)
                    powder = true
                ...
                QBCore.Functions.Notify("Bunun için suda yüzmen lazım.", "error")
            end
        else
            QBCore.Functions.Notify("Temiz görünüyorsun.", "error")
        end
end) 