...
local function GetCurrentCops()
    ...
    for _, v in pairs(players) do
        if v and v.PlayerData.job.name == "fbi" and v.PlayerData.job.onduty then
            amount += 1
        end
    end
    return amount
...
QBCore.Commands.Add("grantlicense", Lang:t("commands.license_grant"), {{name = "id", help = Lang:t('info.player_id')}, {name = "license", help = Lang:t('info.license_type')}}, true, function(source, args)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if Player.PlayerData.job.name == "fbi" and Player.PlayerData.job.grade.level >= Config.LicenseRank then
...
QBCore.Commands.Add("pobject", Lang:t("commands.place_object"), {{name = "type",help = Lang:t("info.poobject_object")}}, true, function(source, args)
    ...
    if Player.PlayerData.job.name == "fbi" and Player.PlayerData.job.onduty then
        ...
        elseif type == "barrier" then
            TriggerClientEvent("Fbi:client:spawnBarrier", src)
        elseif type == "roadsign" then
            TriggerClientEvent("Fbi:client:spawnRoadSign", src)
...
QBCore.Commands.Add("jail", Lang:t("commands.jail_player"), {}, false, function(source)
    ...
    local Player = QBCore.Functions.GetPlayer(src)
    if Player.PlayerData.job.name == "fbi" and Player.PlayerData.job.onduty then
        TriggerClientEvent("Fbi:client:JailPlayer", src)
    else
        TriggerClientEvent('QBCore:Notify', src, Lang:t("error.on_duty_police_only"), 'error')
    end
end)
...
QBCore.Commands.Add('fine', Lang:t("commands.fine"), {{name = 'id', help = Lang:t('info.player_id')}, {name = 'amount', help = Lang:t('info.amount')}}, false, function(source, args)
    ...
    if biller.PlayerData.job.name == "fbi" then
        if billed ~= nil then
            if biller.PlayerData.citizenid ~= billed.PlayerData.citizenid then
                if amount and amount > 0 then
                    ...
                    exports['qb-management']:AddMoney('fbi', amount)
                else
...
QBCore.Commands.Add("takedna", Lang:t("commands.takedna"), {{name = "id", help = Lang:t('info.player_id')}}, true, function(source, args)
    ...
    if not OtherPlayer or Player.PlayerData.job.name ~= "fbi" or not Player.PlayerData.job.onduty then return end
...
QBCore.Commands.Add('911p', Lang:t("commands.police_report"), {{name='message', help= Lang:t("commands.message_sent")}}, false, function(source, args)
    ...
    for _, v in pairs(players) do
        if v and v.PlayerData.job.name == 'fbi' and v.PlayerData.job.onduty then
            ...
            TriggerClientEvent('Fbi:client:policeAlert', v.PlayerData.source, coords, message)
        end
    end
end)
...
QBCore.Functions.CreateUseableItem("moneybag", function(source, item)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if not Player or not Player.Functions.GetItemByName("moneybag") or not item.info or item.info == "" or Player.PlayerData.job.name == "fbi" or not Player.Functions.RemoveItem("moneybag", 1, item.slot) then return end
    Player.Functions.AddMoney("cash", tonumber(item.info.cash), "used-moneybag")
end)

-- Callbacks
QBCore.Functions.CreateCallback('Fbi:server:isPlayerDead', function(_, cb, playerId)
    local Player = QBCore.Functions.GetPlayer(playerId)
    cb(Player.PlayerData.metadata["isdead"])
end)

QBCore.Functions.CreateCallback('Fbi:GetPlayerStatus', function(_, cb, playerId)
    local Player = QBCore.Functions.GetPlayer(playerId)
    local statList = {}
    if Player then
...
QBCore.Functions.CreateCallback('Fbi:GetDutyPlayers', function(_, cb)
    local dutyPlayers = {}
    local players = QBCore.Functions.GetQBPlayers()
    for _, v in pairs(players) do
        if v and v.PlayerData.job.name == "fbi" and v.PlayerData.job.onduty then
...
QBCore.Functions.CreateCallback('Fbi:GetImpoundedVehicles', function(_, cb)
    local vehicles = {}
...
QBCore.Functions.CreateCallback('Fbi:IsPlateFlagged', function(_, cb, plate)
    local retval = false
...
QBCore.Functions.CreateCallback('Fbi:GetCops', function(_, cb)
    local amount = 0
    local players = QBCore.Functions.GetQBPlayers()
    for _, v in pairs(players) do
        if v and v.PlayerData.job.name == "fbi" and v.PlayerData.job.onduty then
...
QBCore.Functions.CreateCallback('Fbi:server:IsPoliceForcePresent', function(_, cb)
    local retval = false
    local players = QBCore.Functions.GetQBPlayers()
    for _, v in pairs(players) do
        if v and v.PlayerData.job.name == "fbi" and v.PlayerData.job.grade.level >= 2 then
            retval = true
            break
        end
    end
...
RegisterNetEvent('Fbi:server:policeAlert', function(text)
    local src = source
    local ped = GetPlayerPed(src)
    local coords = GetEntityCoords(ped)
    local players = QBCore.Functions.GetQBPlayers()
    for _, v in pairs(players) do
        if v and v.PlayerData.job.name == 'fbi' and v.PlayerData.job.onduty then
            ...
            TriggerClientEvent("qb-phone:client:addPoliceAlert", v.PlayerData.source, alertData)
            TriggerClientEvent('Fbi:client:policeAlert', v.PlayerData.source, coords, text)
        end
    end
end)

RegisterNetEvent('Fbi:server:TakeOutImpound', function(plate, garage)
    local src = source
...
RegisterNetEvent('Fbi:server:CuffPlayer', function(playerId, isSoftcuff)
    ...
    if not Player or not CuffedPlayer or (not Player.Functions.GetItemByName("handcuffs") and Player.PlayerData.job.name ~= "fbi") then return end
...
RegisterNetEvent('Fbi:server:BillPlayer', function(playerId, price)
    ...
    local Player = QBCore.Functions.GetPlayer(src)
    local OtherPlayer = QBCore.Functions.GetPlayer(playerId)
    if not Player or not OtherPlayer or Player.PlayerData.job.name ~= "fbi" then return end

    OtherPlayer.Functions.RemoveMoney("bank", price, "paid-bills")
    exports['qb-management']:AddMoney("fbi", price)
...
-- RegisterNetEvent('Fbi:server:FlaggedPlateTriggered', function(camId, plate, street1, street2, blipSettings)
--     local src = source
...
RegisterServerEvent("qb-fbijob:server:Charge", function(citizen, price)
    local biller = QBCore.Functions.GetPlayer(source)
