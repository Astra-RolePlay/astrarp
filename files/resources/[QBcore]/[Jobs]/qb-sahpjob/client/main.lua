...
AddEventHandler('QBCore:Client:OnPlayerLoaded', function()
    ...
    if PlayerJob and PlayerJob.name ~= "sahp" then
        if DutyBlips then
...
RegisterNetEvent('sheriff:client:policeAlert', function(coords, text)
    ...
    QBCore.Functions.Notify({text = text, caption = street1name.. ' ' ..street2name}, 'sahp')
...
Citizen.CreateThread(function()
	...
	{ options = { { event = "illenium-appearance:client:openClothingShopMenu", icon = "fas fa-tshirt", label = "Kıyafet Dükkanı", job = "sahp" }, },
	  distance = 2.0
	})
