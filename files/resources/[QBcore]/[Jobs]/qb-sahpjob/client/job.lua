...
function TakeOutImpound(vehicle)
    ...
    if coords then
		...
		SetVehicleDirtLevel(veh, 0.0)
                ...
                exports['ed-fuel']:SetFuel(veh, vehicle.fuel)
                doCarDamage(veh, vehicle)
                TriggerServerEvent('sahp:server:TakeOutImpound', vehicle.plate, currentGarage)
                closeMenuFull()
...
local function IsArmoryWhitelist() -- being removed
    ...
    if QBCore.Functions.GetPlayerData().job.name == 'sahp' then
        retval = true
    end
    return retval
end
...
function MenuGarage(currentSelection)
    ...
    for veh, label in pairs(authorizedVehicles) do
        vehicleMenu[#vehicleMenu+1] = {
            ...
            txt = "",
            params = {
                event = "sahp:client:TakeOutVehicle",
                args = {
                    vehicle = veh,
...
function MenuImpound(currentSelection)
    ...
    QBCore.Functions.TriggerCallback("sahp:GetImpoundedVehicles", function(result)
        ...
        if result == nil then
            ...
            for _ , v in pairs(result) do
                ...
                impoundMenu[#impoundMenu+1] = {
                    ...
                    params = {
                        event = "sahp:client:TakeOutImpound",
                        args = {
                            vehicle = v,
                            currentSelection = currentSelection
                        }
...
--Events
RegisterNetEvent('sahp:client:showFingerprint', function(playerId)
    openFingerprintUI()
    FingerPrintSessionId = playerId
end)

RegisterNetEvent('sahp:client:showFingerprintId', function(fid)
    SendNUIMessage({
...
RegisterNUICallback('doFingerScan', function(_, cb)
    TriggerServerEvent('sahp:server:showFingerprintId', FingerPrintSessionId)
    cb("ok")
end)

RegisterNetEvent('sahp:client:SendEmergencyMessage', function(coords, message)
    TriggerServerEvent("sahp:server:SendEmergencyMessage", coords, message)
    TriggerEvent("sahp:client:CallAnim")
end)

RegisterNetEvent('sahp:client:EmergencySound', function()
    PlaySound(-1, "Event_Start_Text", "GTAO_FM_Events_Soundset", 0, 0, 1)
end)

RegisterNetEvent('sahp:client:CallAnim', function()
...
RegisterNetEvent('sahp:client:ImpoundVehicle', function(fullImpound, price)
    local vehicle = QBCore.Functions.GetClosestVehicle()
    local bodyDamage = math.ceil(GetVehicleBodyHealth(vehicle))
    local engineDamage = math.ceil(GetVehicleEngineHealth(vehicle))
    local totalFuel = exports['ed-fuel']:GetFuel(vehicle)
    if vehicle ~= 0 and vehicle then
        ...
        if #(pos - vehpos) < 5.0 and not IsPedInAnyVehicle(ped) then
           QBCore.Functions.Progressbar('impound', Lang:t('progressbar.impound'), 5000, false, true, {
                ...
                TriggerServerEvent("sahp:server:Impound", plate, fullImpound, price, bodyDamage, engineDamage, totalFuel)
...
RegisterNetEvent('sahp:client:CheckStatus', function()
    local statusOn = false
    QBCore.Functions.GetPlayerData(function(PlayerData)
        if PlayerData.job.name == "sahp" then
            local player, distance = GetClosestPlayer()
            if player ~= -1 and distance < 5.0 then
                local playerId = GetPlayerServerId(player)
                QBCore.Functions.TriggerCallback('sahp:GetPlayerStatus', function(result)
                    if result then
                        for _, v in pairs(result) do
                            statusOn = true
                            if statusOn then 
                                QBCore.Functions.Notify("GSR: Pozitif", 'success') else statusOn = false end
                        end
                        if not statusOn then QBCore.Functions.Notify("GSR: Negatif", 'error') end
                    end
                end, playerId)
            else
...
RegisterNetEvent("sahp:client:VehicleMenuHeader", function (data)
    MenuGarage(data.currentSelection)
    currentGarage = data.currentSelection
end)


RegisterNetEvent("sahp:client:ImpoundMenuHeader", function (data)
    MenuImpound(data.currentSelection)
    currentGarage = data.currentSelection
end)

RegisterNetEvent('sahp:client:TakeOutImpound', function(data)
    if inImpound then
...
RegisterNetEvent('sahp:client:TakeOutVehicle', function(data)
    if inGarage then
        local vehicle = data.vehicle
...
RegisterNetEvent('sahp:client:EvidenceStashDrawer', function(data)
...
RegisterNetEvent('qb-sahpjob:ToggleDuty', function()
    TriggerServerEvent("QBCore:ToggleDuty")
    TriggerServerEvent("sahp:server:UpdateCurrentCops")
end)

RegisterNetEvent('qb-sahp:client:scanFingerPrint', function()
    local player, distance = GetClosestPlayer()
    if player ~= -1 and distance < 2.5 then
        local playerId = GetPlayerServerId(player)
        TriggerServerEvent("sahp:server:showFingerprint", playerId)
    else
        QBCore.Functions.Notify(Lang:t("error.none_nearby"), "error")
    end
end)

RegisterNetEvent('qb-sahp:client:openArmoury', function()
    ...
    SetWeaponSeries()
    TriggerServerEvent("inventory:server:OpenInventory", "shop", "sahp", authorizedItems)
end)

RegisterNetEvent('qb-sahp:client:spawnHelicopter', function(k)
...
RegisterNetEvent("qb-sahp:client:openStash", function()
    TriggerServerEvent("inventory:server:OpenInventory", "stash", ""..QBCore.Functions.GetPlayerData().citizenid)
    TriggerEvent("inventory:client:SetCurrentStash", "Sahp_"..QBCore.Functions.GetPlayerData().citizenid)
end)

RegisterNetEvent('qb-sahp:client:openTrash', function()
    TriggerServerEvent("inventory:server:OpenInventory", "stash", "Çöp Kutusu", {
        maxweight = 4000000,
        slots = 300,
    })
    TriggerEvent("inventory:client:SetCurrentStash", "Çöp Kutusu")
end)
...
local function stash()
    CreateThread(function()
        while true do
            Wait(0)
            if inStash and PlayerJob.name == "sahp" then
                if PlayerJob.onduty then sleep = 5 end
                if IsControlJustReleased(0, 38) then
                    TriggerServerEvent("inventory:server:OpenInventory", "stash", "Sahp_"..QBCore.Functions.GetPlayerData().citizenid)
                    TriggerEvent("inventory:client:SetCurrentStash", "Sahp_"..QBCore.Functions.GetPlayerData().citizenid)
                    break
                end
            else
...
local function trash()
    CreateThread(function()
        while true do
            Wait(0)
            if inTrash and PlayerJob.name == "sahp" then
                if PlayerJob.onduty then sleep = 5 end
                if IsControlJustReleased(0, 38) then
                    TriggerServerEvent("inventory:server:OpenInventory", "stash", "Çöp Kutusu", {
                        maxweight = 4000000,
...
local function fingerprint()
    CreateThread(function()
        while true do
            Wait(0)
            if inFingerprint and PlayerJob.name == "sahp" then
                if PlayerJob.onduty then sleep = 5 end
                if IsControlJustReleased(0, 38) then
                    TriggerEvent("qb-sahp:client:scanFingerPrint")
                    break
                end
            else
                break
...
local function armoury()
    CreateThread(function()
        while true do
            Wait(0)
            if inArmoury and PlayerJob.name == "sahp" then
                if PlayerJob.onduty then sleep = 5 end
                if IsControlJustReleased(0, 38) then
                    TriggerEvent("qb-sahp:client:openArmoury")
                    break
                end
            else
                break
...
local function heli()
    CreateThread(function()
        while true do
            Wait(0)
            if inHelicopter and PlayerJob.name == "sahp" then
                if PlayerJob.onduty then sleep = 5 end
                if IsControlJustReleased(0, 38) then
                    TriggerEvent("qb-sahp:client:spawnHelicopter")
                    break
                end
            else
                break
...
local function impound()
    CreateThread(function()
        while true do
            Wait(0)
            if inImpound and PlayerJob.name == "sahp" then
                if PlayerJob.onduty then sleep = 5 end
                if IsPedInAnyVehicle(PlayerPedId(), false) then
                    if IsControlJustReleased(0, 38) then
                        QBCore.Functions.DeleteVehicle(GetVehiclePedIsIn(PlayerPedId()))
...
local function garage()
    CreateThread(function()
        while true do
            Wait(0)
            if inGarage and PlayerJob.name == "sahp" then
                if PlayerJob.onduty then sleep = 5 end
                if IsPedInAnyVehicle(PlayerPedId(), false) then
                    if IsControlJustReleased(0, 38) then
                        QBCore.Functions.DeleteVehicle(GetVehiclePedIsIn(PlayerPedId()))
...
if Config.UseTarget then
        ...
        for k, v in pairs(Config.Locations["stash"]) do
            exports['qb-target']:AddBoxZone("SheriffStash_"..k, vector3(v.x, v.y, v.z), 1.5, 1.5, {
                ...
                options = {
                    {
                        type = "client",
                        event = "qb-sahp:client:openStash",
                        icon = "fas fa-lock",
                        label = "Kişisel Depo",
                        job = "sahp",
                    },
        ...
        for k, v in pairs(Config.Locations["trash"]) do
            exports['qb-target']:AddBoxZone("SheriffTrash_"..k, vector3(v.x, v.y, v.z), 1, 1.75, {
                ...
                options = {
                    {
                        type = "client",
                        event = "qb-sahp:client:openTrash",
                        icon = "fas fa-trash",
                        label = "Çöp Kutusu",
                        job = "sahp",
                    },
        ...
        for k, v in pairs(Config.Locations["fingerprint"]) do
            exports['qb-target']:AddBoxZone("PoliceFingerprint_"..k, vector3(v.x, v.y, v.z), 2, 1, {
                ...
                options = {
                    {
                        type = "client",
                        event = "qb-sahp:client:scanFingerPrint",
        ...
        for k, v in pairs(Config.Locations["armory"]) do
            exports['qb-target']:AddBoxZone("PoliceArmory_"..k, vector3(v.x, v.y, v.z), 5, 1, {
                ...
                options = {
                    {
                        type = "client",
                        event = "qb-sahp:client:openArmoury",
                        icon = "fas fa-archive",
                        label = "Cephanelik",
                        job = "sahp",
                    },
    ...
    stashCombo:onPlayerInOut(function(isPointInside, _, _)
        if isPointInside then
            inStash = true
            if PlayerJob.name == 'sahp' and PlayerJob.onduty then
                ...
                exports['qb-menu']:showHeader({
                    {
                        header = Lang:t('info.evidence_stash', {value = currentEvidence}),
                        params = {
                            event = 'sahp:client:EvidenceStashDrawer',
                            args = {
                    ...
                    exports['qb-menu']:showHeader({
                        {
                            ...
                            params = {
                                event = 'sahp:client:ImpoundMenuHeader',
                                args = {
                                    currentSelection = currentSelection,
                                }
                            }
