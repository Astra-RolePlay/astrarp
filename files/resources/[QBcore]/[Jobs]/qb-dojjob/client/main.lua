...
AddEventHandler('QBCore:Client:OnPlayerLoaded', function()
    ...
    if PlayerJob and PlayerJob.name ~= "doj" then
        if DutyBlips then
...
RegisterNetEvent('doj:client:sendBillingMail', function(amount)
...
RegisterNetEvent('doj:client:policeAlert', function(coords, text)
    local street1, street2 = GetStreetNameAtCoord(coords.x, coords.y, coords.z)
    local street1name = GetStreetNameFromHashKey(street1)
    local street2name = GetStreetNameFromHashKey(street2)
    QBCore.Functions.Notify({text = text, caption = street1name.. ' ' ..street2name}, 'doj')
...
RegisterNetEvent('doj:client:SendToJail', function(time)
    TriggerServerEvent("doj:server:SetHandcuffStatus", false)
    isHandcuffed = false
    isEscorted = false
    ClearPedTasks(PlayerPedId())
...
RegisterNetEvent('doj:client:SendPoliceEmergencyAlert', function()
    local Player = QBCore.Functions.GetPlayerData()
    
    TriggerServerEvent('doj:server:policeAlert', Lang:t('info.officer_down', {lastname = Player.charinfo.lastname, callsign = Player.metadata.callsign}))
...
Citizen.CreateThread(function()
	...
	{ options = { { event = "illenium-appearance:client:openClothingShopMenu", icon = "fas fa-tshirt", label = "Kıyafet Dükkanı", job = "doj" }, },
	  distance = 2.0
	})
