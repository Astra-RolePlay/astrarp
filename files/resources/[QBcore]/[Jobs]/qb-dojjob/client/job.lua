...
function TakeOutImpound(vehicle)
    ...
    if coords then
		...
		SetVehicleDirtLevel(veh, 0.0)
                ...
                TriggerServerEvent('doj:server:TakeOutImpound', vehicle.plate, currentGarage)
                closeMenuFull()
...
local function IsArmoryWhitelist() -- being removed
    ...
    if QBCore.Functions.GetPlayerData().job.name == 'doj' then
        retval = true
    end
    return retval
end
...
function MenuGarage(currentSelection)
    ...
    for veh, label in pairs(authorizedVehicles) do
        vehicleMenu[#vehicleMenu+1] = {
            ...
            txt = "",
            params = {
                event = "doj:client:TakeOutVehicle",
                args = {
                    vehicle = veh,
...
function MenuImpound(currentSelection)
    ...
    QBCore.Functions.TriggerCallback("doj:GetImpoundedVehicles", function(result)
        ...
        if result == nil then
            ...
            for _ , v in pairs(result) do
                ...
                impoundMenu[#impoundMenu+1] = {
                    ...
                    params = {
                        event = "doj:client:TakeOutImpound",
                        args = {
                            vehicle = v,
                            currentSelection = currentSelection
                        }
...
--Events
RegisterNetEvent('doj:client:showFingerprint', function(playerId)
    openFingerprintUI()
    FingerPrintSessionId = playerId
end)

RegisterNetEvent('doj:client:showFingerprintId', function(fid)
    SendNUIMessage({
...
RegisterNUICallback('doFingerScan', function(_, cb)
    TriggerServerEvent('doj:server:showFingerprintId', FingerPrintSessionId)
    cb("ok")
end)

RegisterNetEvent('doj:client:SendEmergencyMessage', function(coords, message)
    TriggerServerEvent("doj:server:SendEmergencyMessage", coords, message)
    TriggerEvent("doj:client:CallAnim")
end)

RegisterNetEvent('doj:client:EmergencySound', function()
    PlaySound(-1, "Event_Start_Text", "GTAO_FM_Events_Soundset", 0, 0, 1)
end)

RegisterNetEvent('doj:client:CallAnim', function()
...
RegisterNetEvent('doj:client:ImpoundVehicle', function(fullImpound, price)
    ...
    if vehicle ~= 0 and vehicle then
        ...
        if #(pos - vehpos) < 5.0 and not IsPedInAnyVehicle(ped) then
           QBCore.Functions.Progressbar('impound', Lang:t('progressbar.impound'), 5000, false, true, {
                ...
                TriggerServerEvent("doj:server:Impound", plate, fullImpound, price, bodyDamage, engineDamage, totalFuel)
...
RegisterNetEvent('doj:client:CheckStatus', function()
    ...
    QBCore.Functions.GetPlayerData(function(PlayerData)
        if PlayerData.job.name == "doj" then
            ...
            if player ~= -1 and distance < 5.0 then
                ...
                QBCore.Functions.TriggerCallback('doj:GetPlayerStatus', function(result)
                    if result then
                        for _, v in pairs(result) do
                            statusOn = true
                            if statusOn then 
...
RegisterNetEvent("doj:client:VehicleMenuHeader", function (data)
    MenuGarage(data.currentSelection)
    currentGarage = data.currentSelection
end)


RegisterNetEvent("doj:client:ImpoundMenuHeader", function (data)
    MenuImpound(data.currentSelection)
    currentGarage = data.currentSelection
end)

RegisterNetEvent('doj:client:TakeOutImpound', function(data)
    if inImpound then
...
RegisterNetEvent('doj:client:TakeOutVehicle', function(data)
    if inGarage then
        local vehicle = data.vehicle
...
RegisterNetEvent('doj:client:EvidenceStashDrawer', function(data)
...
RegisterNetEvent('qb-dojjob:ToggleDuty', function()
    TriggerServerEvent("QBCore:ToggleDuty")
    TriggerServerEvent("doj:server:UpdateCurrentCops")
end)

RegisterNetEvent('qb-doj:client:scanFingerPrint', function()
    local player, distance = GetClosestPlayer()
    if player ~= -1 and distance < 2.5 then
        local playerId = GetPlayerServerId(player)
        TriggerServerEvent("doj:server:showFingerprint", playerId)
    else
        QBCore.Functions.Notify(Lang:t("error.none_nearby"), "error")
    end
end)

RegisterNetEvent('qb-doj:client:openArmoury', function()
    ...
    SetWeaponSeries()
    TriggerServerEvent("inventory:server:OpenInventory", "shop", "doj", authorizedItems)
end)

RegisterNetEvent('qb-doj:client:spawnHelicopter', function(k)
...
RegisterNetEvent("qb-doj:client:openStash", function()
    TriggerServerEvent("inventory:server:OpenInventory", "stash", ""..QBCore.Functions.GetPlayerData().citizenid)
    TriggerEvent("inventory:client:SetCurrentStash", "DOJ_"..QBCore.Functions.GetPlayerData().citizenid)
end)

RegisterNetEvent('qb-doj:client:openTrash', function()
...
local function stash()
    CreateThread(function()
        while true do
            Wait(0)
            if inStash and PlayerJob.name == "doj" then
                if PlayerJob.onduty then sleep = 5 end
                if IsControlJustReleased(0, 38) then
                    TriggerServerEvent("inventory:server:OpenInventory", "stash", "DOJ_"..QBCore.Functions.GetPlayerData().citizenid)
                    TriggerEvent("inventory:client:SetCurrentStash", "DOJ_"..QBCore.Functions.GetPlayerData().citizenid)
                    break
                end
            else
...
local function trash()
    CreateThread(function()
        while true do
            Wait(0)
            if inTrash and PlayerJob.name == "doj" then
                if PlayerJob.onduty then sleep = 5 end
                if IsControlJustReleased(0, 38) then
                    TriggerServerEvent("inventory:server:OpenInventory", "stash", "Çöp Kutusu", {
                        maxweight = 4000000,
...
local function fingerprint()
    CreateThread(function()
        while true do
            Wait(0)
            if inFingerprint and PlayerJob.name == "doj" then
                if PlayerJob.onduty then sleep = 5 end
                if IsControlJustReleased(0, 38) then
                    TriggerEvent("qb-doj:client:scanFingerPrint")
                    break
                end
            else
                break
...
local function armoury()
    CreateThread(function()
        while true do
            Wait(0)
            if inArmoury and PlayerJob.name == "doj" then
                if PlayerJob.onduty then sleep = 5 end
                if IsControlJustReleased(0, 38) then
                    TriggerEvent("qb-doj:client:openArmoury")
                    break
                end
            else
                break
...
local function heli()
    CreateThread(function()
        while true do
            Wait(0)
            if inHelicopter and PlayerJob.name == "doj" then
                if PlayerJob.onduty then sleep = 5 end
                if IsControlJustReleased(0, 38) then
                    TriggerEvent("qb-doj:client:spawnHelicopter")
                    break
                end
            else
                break
...
local function impound()
    CreateThread(function()
        while true do
            Wait(0)
            if inImpound and PlayerJob.name == "doj" then
                if PlayerJob.onduty then sleep = 5 end
                if IsPedInAnyVehicle(PlayerPedId(), false) then
                    if IsControlJustReleased(0, 38) then
                        QBCore.Functions.DeleteVehicle(GetVehiclePedIsIn(PlayerPedId()))
...
local function garage()
    CreateThread(function()
        while true do
            Wait(0)
            if inGarage and PlayerJob.name == "doj" then
                if PlayerJob.onduty then sleep = 5 end
                if IsPedInAnyVehicle(PlayerPedId(), false) then
                    if IsControlJustReleased(0, 38) then
                        QBCore.Functions.DeleteVehicle(GetVehiclePedIsIn(PlayerPedId()))
...
if Config.UseTarget then
        ...
        for k, v in pairs(Config.Locations["fingerprint"]) do
            exports['qb-target']:AddBoxZone("PoliceFingerprint_"..k, vector3(v.x, v.y, v.z), 2, 1, {
                ...
                options = {
                    {
                        type = "client",
                        event = "qb-doj:client:scanFingerPrint",
        ...
        for k, v in pairs(Config.Locations["armory"]) do
            exports['qb-target']:AddBoxZone("PoliceArmory_"..k, vector3(v.x, v.y, v.z), 5, 1, {
                ...
                options = {
                    {
                        type = "client",
                        event = "qb-doj:client:openArmoury",
                        icon = "fas fa-archive",
                        label = "Cephanelik",
                        job = "doj",
                    },
    ...
    stashCombo:onPlayerInOut(function(isPointInside, _, _)
        if isPointInside then
            inStash = true
            if PlayerJob.name == 'doj' and PlayerJob.onduty then
                ...
                exports['qb-menu']:showHeader({
                    {
                        header = Lang:t('info.evidence_stash', {value = currentEvidence}),
                        params = {
                            event = 'doj:client:EvidenceStashDrawer',
                            args = {
                    ...
                    exports['qb-menu']:showHeader({
                        {
                            ...
                            params = {
                                event = 'doj:client:ImpoundMenuHeader',
                                args = {
                                    currentSelection = currentSelection,
                                }
                            }
