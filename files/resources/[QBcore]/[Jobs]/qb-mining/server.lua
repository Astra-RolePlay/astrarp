...
RegisterNetEvent('qb-mining:server:toggleItem', function(give, item, amount)
	local src = source
	if give == 0 or give == false then
		...
		else TriggerEvent("qb-mining:server:DupeWarn", item, src) end -- if not boot the player
	else
...
RegisterNetEvent("qb-mining:server:DupeWarn", function(item, newsrc)
