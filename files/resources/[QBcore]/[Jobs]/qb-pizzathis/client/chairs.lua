CreateThread(function()
	for k, v in pairs(Config.Chairs) do
		exports['qb-target']:AddBoxZone("PizzChair"..k, v.location, v.width, v.depth, { name="PizzChair"..k, heading = v.heading, debugPoly=false, minZ = v.minZ, maxZ = v.maxZ, }, 
			{ options = { { event = "qb-pizzathis:Chair", icon = "fas fa-chair", label = "Otur", loc = v.location, head = v.heading, seat = v.seat }, },
			  distance = v.distance
		})
	end
end)

RegisterNetEvent('qb-pizzathis:Chair')
AddEventHandler('qb-pizzathis:Chair', function(data)
	...
	while true do
		...
		if sitting then
			if IsControlJustReleased(0, 202) then
				...
				if pseat == 1 then SetEntityCoords(ped, vector3(808.69, -751.55, 26.78-0.5)) end
				if pseat == 2 then SetEntityCoords(ped, vector3(805.09, -751.64, 26.78-0.5)) end
				if pseat == 3 then SetEntityCoords(ped, vector3(801.11, -751.54, 26.78-0.5)) end
				if pseat == 4 then SetEntityCoords(ped, vector3(797.26, -751.56, 26.78-0.5)) end
				if pseat == 5 then SetEntityCoords(ped, vector3(799.7, -748.8, 26.78-0.5)) end
				if pseat == 6 then SetEntityCoords(ped, vector3(796.46, -748.66, 26.78-0.5)) end
				if pseat == 7 then SetEntityCoords(ped, vector3(795.42, -749.88, 26.78-0.5)) end
				if pseat == 8 then SetEntityCoords(ped, vector3(795.34, -753.39, 26.78-0.5)) end
				if pseat == 9 then SetEntityCoords(ped, vector3(799.53, -753.53, 26.78-0.5)) end
				if pseat == 10 then SetEntityCoords(ped, vector3(798.94, -756.07, 26.78-0.5)) end
				
				if pseat == 11 then SetEntityCoords(ped, vector3(808.29, -753.46, 26.78-0.5)) end
				if pseat == 12 then SetEntityCoords(ped, vector3(807.06, -753.62, 26.78-0.5)) end
				if pseat == 13 then SetEntityCoords(ped, vector3(806.31, -753.61, 26.78-0.5)) end
				if pseat == 14 then SetEntityCoords(ped, vector3(804.93, -753.63, 26.78-0.5)) end
				if pseat == 15 then SetEntityCoords(ped, vector3(804.14, -753.59, 26.78-0.5)) end
				if pseat == 16 then SetEntityCoords(ped, vector3(802.9, -753.61, 26.78-0.5)) end
				if pseat == 17 then SetEntityCoords(ped, vector3(802.11, -753.59, 26.78-0.5)) end
				if pseat == 18 then SetEntityCoords(ped, vector3(800.74, -753.63, 26.78-0.5)) end
				if pseat == 19 then SetEntityCoords(ped, vector3(798.24, -757.15, 26.78-0.5)) end
				if pseat == 20 then SetEntityCoords(ped, vector3(798.36, -758.28, 26.78-0.5)) end
				if pseat == 21 then SetEntityCoords(ped, vector3(798.32, -759.04, 26.78-0.5)) end
				if pseat == 22 then SetEntityCoords(ped, vector3(798.36, -760.38, 26.78-0.5)) end
				
				if pseat == 23 then SetEntityCoords(ped, vector3(796.12, -747.16, 26.78-0.5)) end
				if pseat == 24 then SetEntityCoords(ped, vector3(799.78, -747.05, 26.78-0.5)) end
				
				if pseat == 25 then SetEntityCoords(ped, vector3(796.33, -741.25, 26.78-0.5)) end
				if pseat == 26 then SetEntityCoords(ped, vector3(800.3, -741.5, 26.78-0.5)) end
				
				if pseat == 27 then SetEntityCoords(ped, vector3(802.65, -741.44, 26.78-0.5)) end
				if pseat == 28 then SetEntityCoords(ped, vector3(806.46, -741.27, 26.78-0.5)) end
				
				if pseat == 29 then SetEntityCoords(ped, vector3(808.05, -741.27, 26.78-0.5)) end
				if pseat == 30 then SetEntityCoords(ped, vector3(811.95, -741.23, 26.78-0.5)) end
				
				if pseat == 32 then SetEntityCoords(ped, vector3(812.85, -747.13, 26.78-0.5)) end
				if pseat == 31 then SetEntityCoords(ped, vector3(809.14, -747.22, 26.78-0.5)) end
				
				
				if pseat == 33 then SetEntityCoords(ped, vector3(803.35, -761.66, 31.27-0.5)) end
				if pseat == 34 then SetEntityCoords(ped, vector3(806.01, -761.49, 31.27-0.5)) end
				
				if pseat == 35 then SetEntityCoords(ped, vector3(798.56, -750.4, 31.27-0.5)) end
				if pseat == 36 then SetEntityCoords(ped, vector3(798.86, -753.3, 31.27-0.5)) end
				if pseat == 37 then SetEntityCoords(ped, vector3(797.04, -753.41, 31.27-0.5)) end
				
				if pseat == 38 then SetEntityCoords(ped, vector3(800.41, -756.39, 31.27-0.5)) end
...
Config.Chairs = {
	...
	{ location = vector3(799.51, -756.1, 26.78), heading = 7.0, width = 0.6, depth = 0.6, minZ=25.78, maxZ=26.78, distance = 1.7, saet = 10 },
