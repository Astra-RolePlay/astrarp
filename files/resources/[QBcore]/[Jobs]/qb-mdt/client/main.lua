...
RegisterKeyMapping('mdt', 'Open Police MDT', 'keyboard', 'f10')
...
RegisterNetEvent('mdt:client:open', function(bulletin, activeUnits, calls, cid)
    ...
    if not zone then zone = "BİLİNMİYOR" end;
    ...
    SendNUIMessage({ type = "data", activeUnits = activeUnits, citizenid = cid, ondutyonly = Config.OnlyShowOnDuty, name = "Hoş geldin, " ..PlayerData.job.grade.name..' '..PlayerData.charinfo.lastname:sub(1,1):upper()..PlayerData.charinfo.lastname:sub(2), location = playerStreetsLocation, fullname = PlayerData.charinfo.firstname..' '..PlayerData.charinfo.lastname, bulletin = bulletin })
...
RegisterNetEvent('mdt:client:setRadio', function(radio)
    if type(tonumber(radio)) == "number" then
        ...
        QBCore.Functions.Notify("Telsiz "..radio.." frekansına ayarlandı.", "success")
    else
        QBCore.Functions.Notify("Geçersiz frekans kodu!", "error")
    end
end)
...
RegisterNUICallback("dispatchNotif", function(data, cb)
    ...
    if mentioned then
        ...
        TriggerEvent('erp_phone:sendNotification', {img = info['profilepic'], title = "İhbar (Kişiye Özel)", content = info['message'], time = 7500, customPic = true })

        PlaySoundFrontend(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", false)
        PlaySoundFrontend(-1, "Event_Start_Text", "GTAO_FM_Events_Soundset", 0)
    else
        TriggerEvent('erp_phone:sendNotification', {img = info['profilepic'], title = "İhbar ("..info['name']..")", content = info['message'], time = 5000, customPic = true })
    end
    cb(true)
end)
...
RegisterNUICallback("impoundVehicle", function(data, cb)
    ...
    if JobType == 'police' then
        ...
        local bodyDamage = math.ceil(GetVehicleBodyHealth(found))
        local engineDamage = math.ceil(GetVehicleEngineHealth(found))
        local totalFuel = exports['ed-fuel']:GetFuel(found)
        TriggerServerEvent("police:server:Impound", data["plate"], true, 0, bodyDamage, engineDamage, totalFuel, data["time"])
