...
Citizen.CreateThread(function()
    ...
    if Config.sql == "oxmysql" then  
        MySQL.update('UPDATE qb_fishing SET tasks = ?, time = NULL WHERE time <= CURDATE()', { json.encode(tasks) })
    elseif Config.sql == "ghmattimysql" then  
        exports.ghmattimysql:execute("UPDATE qb_fishing SET tasks = @tasks, time = NULL WHERE time <= CURDATE()", {
            ['@tasks'] = json.encode(tasks)
        })   
    elseif Config.sql == "mysql-async" then 
        MySQL.Async.execute('UPDATE qb_fishing SET tasks = @tasks, time = NULL WHERE time <= CURDATE()', {
            ['@tasks'] = json.encode(tasks)
        })
    end
end)
...
QBCore.Functions.CreateCallback('ak4y-advancedFishing:getPlayerDetails', function(source, cb)
    ...
    local callbackData = {}
    if Config.sql == "oxmysql" then  
        local result = MySQL.query.await('SELECT * FROM qb_fishing WHERE citizenid = ?', { citizenId })
        if result[1] == nil then 
            MySQL.insert('INSERT INTO qb_fishing (citizenid, currentXP, tasks) VALUES (?, ?, ?)', {
                citizenId,
                0,
                json.encode(tasks)
            })
        ...
        cb(callbackData)
    elseif Config.sql == "ghmattimysql" then  
        exports.ghmattimysql:execute("SELECT * FROM qb_fishing WHERE citizenid = @citizenid", {
            ["@citizenid"] = citizenId
        }, function (result)
            if result[1] == nil then 
                exports.ghmattimysql:execute("INSERT INTO qb_fishing SET citizenid = @citizenid, currentXP = @currentXP, tasks = @tasks", {
                    ["@citizenid"] = citizenId,
                    ["@currentXP"] = 0,
                    ["@tasks"] = json.encode(tasks),
                })
    ...
    elseif Config.sql == "mysql-async" then 
        MySQL.Async.fetchAll("SELECT * FROM qb_fishing WHERE citizenid = @citizenid", {
            ["@citizenid"] = citizenId
        }, function (result)
            if result[1] == nil then 
                MySQL.Async.execute('INSERT INTO qb_fishing (citizenid, currentXP, tasks) VALUES (@citizenid, @currentXP, @tasks)', {
                    ["@citizenid"] = citizenId,
                    ["@currentXP"] = 0,
                    ["@tasks"] = json.encode(tasks)
                })
...
QBCore.Functions.CreateCallback('ak4y-advancedFishing:rewards', function(source, cb, data)
    ...
    if Config.sql == "oxmysql" then  
        local result = MySQL.query.await('SELECT tasks FROM qb_fishing WHERE citizenid = ?', { citizenId })
        if result[1] then 
            ...
            if not resultak4y[taskId].taken then 
                if resultak4y[taskId].hasCount >= resultak4y[taskId].requiredCount then 
                    ...
                    MySQL.update('UPDATE qb_fishing SET tasks = ?, time = (CURDATE() + INTERVAL '..Config.Settings.tasksResetDay..' DAY) WHERE citizenid = ?', { json.encode(resultak4y), citizenId })
                    cb(true)
                else
                    cb(false)
                end
    ...
    elseif Config.sql == "ghmattimysql" then  
        exports.ghmattimysql:execute("SELECT tasks FROM qb_fishing WHERE citizenid = @citizenid", {
            ["@citizenid"] = citizenId
        }, function (result)
            if result[1] then 
                local resultak4y = json.decode(result[1].tasks)
                if not resultak4y[taskId].taken then 
                    if resultak4y[taskId].hasCount >= resultak4y[taskId].requiredCount then 
                        resultak4y[taskId].taken = true
                        xPlayer.Functions.AddMoney('cash', money)
                        exports.ghmattimysql:execute("UPDATE qb_fishing SET tasks = @tasks, time = (CURDATE() + INTERVAL "..Config.Settings.tasksResetDay.." DAY) WHERE citizenid = @citizenid", {
                            ['@citizenid'] = citizenId,
    ...
    elseif Config.sql == "mysql-async" then 
        MySQL.Async.fetchAll("SELECT tasks FROM qb_fishing WHERE citizenid = @citizenid", {
            ["@citizenid"] = citizenId
        }, function (result)
            if result[1] then 
                local resultak4y = json.decode(result[1].tasks)
                if not resultak4y[taskId].taken then 
                    if resultak4y[taskId].hasCount >= resultak4y[taskId].requiredCount then 
                        resultak4y[taskId].taken = true
                        xPlayer.Functions.AddMoney('cash', money)
                        MySQL.Async.execute("UPDATE qb_fishing SET tasks = @tasks, time = (CURDATE() + INTERVAL "..Config.Settings.tasksResetDay.." DAY) WHERE citizenid = @citizenid", {
                            ['@citizenid'] = citizenId,
                            ['@tasks'] = json.encode(resultak4y)
                        })
                        cb(true)
...
AddEventHandler('ak4y-advancedFishing:addTaskCount', function(itemName)
    ...
    if Config.sql == "oxmysql" then  
        ...
        if result[1] then 
            ...
            if kayit then 
                MySQL.update('UPDATE qb_fishing SET tasks = ? WHERE citizenid = ?', { json.encode(resultak4y), citizenId })
            end
        end
    elseif Config.sql == "ghmattimysql" then  
        exports.ghmattimysql:execute("SELECT tasks FROM qb_fishing WHERE citizenid = @citizenid", {
            ...
            if result[1] then 
                ...
                if kayit then 
                    exports.ghmattimysql:execute("UPDATE qb_fishing SET tasks = @tasks WHERE citizenid = @citizenid", {
                        ['@citizenid'] = citizenId,
                        ['@tasks'] = json.encode(resultak4y)
                    })
                end
    ...
    elseif Config.sql == "mysql-async" then 
        MySQL.Async.fetchAll("SELECT tasks FROM qb_fishing WHERE citizenid = @citizenid", {
            ...
            if result[1] then 
                ...
                if kayit then 
                    MySQL.Async.execute("UPDATE qb_fishing SET tasks = @tasks WHERE citizenid = @citizenid", {
                        ['@citizenid'] = citizenId,
                        ['@tasks'] = json.encode(resultak4y)
                    })
                end
...
QBCore.Functions.CreateCallback('ak4y-advancedFishing:addXP', function(source, cb, amount)
    ...
    if Config.sql == "oxmysql" then  
        MySQL.update('UPDATE qb_fishing SET currentXP = currentXP + ? WHERE citizenid = ?', { deger, citizenId })
    elseif Config.sql == "ghmattimysql" then  
        exports.ghmattimysql:execute("UPDATE qb_fishing SET currentXP = currentXP + @currentXP WHERE citizenid = @citizenid", {
            ['@citizenid'] = citizenId,
            ['@currentXP'] = deger,
        })
    elseif Config.sql == "mysql-async" then 
        MySQL.Async.execute("UPDATE qb_fishing SET currentXP = currentXP + @currentXP WHERE citizenid = @citizenid", {
            ['@citizenid'] = citizenId,
            ['@currentXP'] = deger,
        })
    end
...
QBCore.Functions.CreateCallback('ak4y-advancedFishing:buyItem', function(source, cb, data)
    ...
    if cashPara >= requiredCash then
        if xPlayer.Functions.AddItem(currentItem.itemName, buyItemCount) then 
            local alinanesya = currentItem.itemName
            if xPlayer.Functions.RemoveMoney('cash', requiredCash) then 
                TriggerClientEvent("QBCore:Notify", src, Config.Language.succesBuy.." "..buyItemCount.."x - "..QBCore.Shared.Items[alinanesya].label.."!", "success")
                cb(true)
            else
                TriggerClientEvent("QBCore:Notify", src, Config.Language.youDontHaveMoney, "error")
                cb(false)
...
QBCore.Functions.CreateCallback('ak4y-advancedFishing:sellItem', function(source, cb, data)
    ...
    if currentAmount ~= nil and currentAmount.amount >= sellItemCount then 
        local satilanesya = currentItem.itemName
        xPlayer.Functions.RemoveItem(currentItem.itemName, sellItemCount)
        TriggerClientEvent("QBCore:Notify", src, Config.Language.succesSold.. " "..sellItemCount..'x, '..QBCore.Shared.Items[satilanesya].label..' - $'..rewardsCash..'!', "success")
