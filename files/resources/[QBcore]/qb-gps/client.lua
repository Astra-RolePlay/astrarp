...
RegisterNetEvent("qb-gps:update")
AddEventHandler("qb-gps:update", function(data)
...
AddEventHandler('qb:playerdead', function(dead)
	if activeGps and dead then 
		TriggerServerEvent("qb-gps:acikgps-kapat", true)
	end
end)

RegisterNetEvent('qb-emerhencyblips:forceClose')
AddEventHandler('qb-emerhencyblips:forceClose', function()
	if activeGps then 
		TriggerServerEvent("qb-gps:acikgps-kapat", false)
	end
end)

RegisterNetEvent('qb-emergencyblips:updateAllData')
AddEventHandler('qb-emergencyblips:updateAllData', function(pData, cData)
	blips = pData
	carBlips = cData
end)

RegisterNetEvent('qb-emergencyblips:removePlayerGps')
AddEventHandler('qb-emergencyblips:removePlayerGps', function(src, pData, cData)
	blips = pData
	carBlips = cData
	Citizen.Wait(500)
...
RegisterNetEvent('qb-emergencyblips:toggle')
AddEventHandler('qb-emergencyblips:toggle', function(active, data, policeBlipData)
...
RegisterNetEvent('qb-closest-police')
AddEventHandler('qb-closest-police', function(cb) 
	local closestPolice = 0
	local policeCount = 0
	for src, info in pairs(blips) do
		if info.blipColor == 79 then
...
Citizen.CreateThread(function()
	while true do
		if activeGps then
			local allBlips = exports["qb-gps"]:GetPlayerCoordsData()
			for src, info in pairs(blips) do
...
local lastBlipType = 41
Citizen.CreateThread(function()
	while true do
		if activeGps and policeBlip then
			local playerPed = PlayerPedId()
			blipType = IsVehicleSirenOn(vehicle) and 1 or GetVehicleClass(vehicle) == 42 and 42 or 41
			if IsPedInAnyVehicle(PlayerPedId()) then
				if not QBCore.Functions.GetPlayerData().metadata["isdead"] then 
					blipScale = 0.85
					local updatedBlip = false
					TriggerServerEvent("qb-emergencyblips:updatePlayerGps", false, 41, true, blipType, blipScale)
					local vehicle = GetVehiclePedIsIn(PlayerPedId())
					blipType = IsVehicleSirenOn(vehicle) and 1 or GetVehicleClass(vehicle) == 15 and 15 or 41
...
RegisterNetEvent("qb-emergencyblip:ac")
AddEventHandler("qb-emergencyblip:ac", function()
    ...
    if hasitem then
			if PlayerData.job ~= nil and PlayerData.job.name ~= 'unemployed' then 
                local keyboard = exports['qb-input']:ShowInput({
                    ...
                    inputs = {
                        ...
                        },
						{
							...
							{ value = "pd", text = "LSPD" }, -- Options MUST include a value and a text option
							...
							{ value = "ms", text = "SAMS" }, -- Options MUST include a value and a text option
							}
                    	}
					}
                })
                ...
                if PlayerData.job.name == "police" or PlayerData.job.name == "ambulance" or PlayerData.job.name == "sasp" or PlayerData.job.name == "sheriff" then
                    TriggerServerEvent('qb-gps:polis-ekle', number, tostring(gpscolor))
                end
			else
...
RegisterNetEvent("qb-emergencyblip:kapat")
AddEventHandler("qb-emergencyblip:kapat", function()
	TriggerServerEvent('qb-gps:acikgps-kapat', false)
end)

local actimkapattim = false
RegisterNetEvent('qb-emergencyblip:ackapa', function()
	if actimkapattim then
		TriggerEvent("qb-emergencyblip:kapat")
		actimkapattim = false
	else
		TriggerEvent("qb-emergencyblip:ac")
		actimkapattim = true
	end
end)