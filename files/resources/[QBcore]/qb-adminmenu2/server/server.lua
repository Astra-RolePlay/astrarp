...
--- Event [Nui]
RegisterNetEvent('qb-adminmenu:server:getMoney', function(moneytype)
    local src = source
    local result =  MySQL.query.await('SELECT money FROM players', {})
        if result then
            ...
            TriggerClientEvent('qb-adminmenu:client:getMoney', src, totalMoney, moneytype)
        end
end)

RegisterNetEvent('qb-adminmenu:server:getPlayerProfile', function(player)
    ...
    TriggerClientEvent('qb-adminmenu:client:getPlayerProfile',source,data)
end)
