...
RegisterNetEvent('qb-adminmenu:client:getPlayerProfile', function(data)
    SendNUIMessage({type = "playerprofile", data = data})
end)

RegisterNetEvent('qb-adminmenu:client:getMoney', function(totalmoney,moneytype)
    SendNUIMessage({type = "money", totalmoney = totalmoney, moneytype = moneytype})
end)


--- Callback [Nui]
RegisterNUICallback("qb-adminmenu:nuicallback:getPlayers", function()
...
RegisterNUICallback("qb-adminmenu:nuicallback:setMinPage", function()
    SetNuiFocusKeepInput(true)
    nuiWalks(true)
end)

RegisterNUICallback("qb-adminmenu:nuicallback:getPlayerProfile", function(player)
    TriggerServerEvent('qb-adminmenu:server:getPlayerProfile',player)
end)

RegisterNUICallback('qb-adminmenu:nuicallback:getVehicles', function()
    SendNUIMessage({type="vehicles",vehicles=vehicles})
end)

RegisterNUICallback('qb-adminmenu:nuicallback:getWeapons', function()
    SendNUIMessage({type="weapons",weapons=weapons})
end)

RegisterNUICallback('qb-adminmenu:nuicallback:weather', function(weather)
...
RegisterNUICallback('qb-adminmenu:nuicallback:time', function(time)
    TriggerServerEvent('qb-weathersync:server:setWeather', time, time)
    QBCore.Functions.Notify(Lang:t("time.changed", {time = time}))
end)

RegisterNUICallback("qb-adminmenu:nuicallback:toggleMutePlayer", function(player)
...
RegisterNUICallback("qb-adminmenu:nuicallback:event", function(data)
...
RegisterNUICallback("qb-adminmenu:nuicallback:toFullPage", function()
    nuiWalks(false)
    SetNuiFocusKeepInput(false)
end)

RegisterNUICallback("qb-adminmenu:nuicallback:escapeNui", function()
    nuiWalks(false)
    SetNuiFocusKeepInput(false)
