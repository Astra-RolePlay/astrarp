if IsDuplicityVersion() then -- Check if server sided
    if Scully.Framework == 'qb-core' then
        ...
        --         if Player.PlayerData.metadata.callsign == 'NO CALLSIGN' then
        --             return '', string.sub(Player.PlayerData.charinfo.firstname, 1, 1) .. '. ' .. Player.PlayerData.charinfo.lastname
        --         else
        --             return Player.PlayerData.metadata.callsign .. ' | ', string.sub(Player.PlayerData.charinfo.firstname, 1, 1) .. '. ' .. xPlayer.PlayerData.charinfo.lastname
        --         end
        --     end,
        -- }
