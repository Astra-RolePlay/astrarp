...
function Startup()
	...
	SendNUIMessage({ action = 'setVoiceDistance', value = LocalPlayer.state.proximity and
	LocalPlayer.state.proximity.mode or "Normal" })
	...
	while true do
		...
		if IsPedInAnyVehicle(player, false) and vehicleIsOn then
			...
			if vehicleSpeed > Config.vehicle.maxSpeed then
				...
				math.ceil(math.ceil(Config.vehicle.maxSpeed * 205) / Config.vehicle.maxSpeed))
			else
			...
			vehicleFuel = exports['ed-fuel']:GetFuel(vehicle)
...
CreateThread(function()
	...
	while true do
		...
		if Config.ui.showStress then
			...
			playerStatus['status'][showPlayerStatus] = {
				name = 'stress',
				value = math.floor(PlayerData.metadata["stress"] > 1 and
				(PlayerData.metadata["stress"] / PlayerData.metadata["stress"]) * 100 or 1)
			}
		end
...
exports('createStatus', function(args)
	local statusCreation = { action = 'createStatus', status = args['status'], color = args['color'], icon = args
	['icon'] }
