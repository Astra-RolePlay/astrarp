...
function FoundTarget(item) {
    if (item.data) {
        ...
        $("#target-eye").attr("class", "fa-regular fa-eye");  
    }
    $("#target-eye").css("color", "rgb(124, 255, 174)");
}
...
function OpenTarget() {
    ...
    $("#target-eye").attr("class", "fa-solid fa-eye-low-vision");
    $("#target-eye").css("color", "rgb(248, 143, 117)");
}
...
function ValidTarget(item) {
    ...
    $.each(item.data, function(index, item) {
        ...
        $("#target-" + index).hover((e) => {
            $("#target-" + index).css("color", e.type === "mouseenter" ? "rgb(124, 255, 174)" : "white")
        })
