...
RegisterCommand("targetfix", function()
	SetNuiFocus(false, false)
	SetNuiFocusKeepInput(false)
	hasFocus = false
end)
...
local function EnableTarget()
	...
	if IsPedArmed(PlayerPedId(), 6) or IsPedInAnyVehicle(PlayerPedId(),false) or QBCore.Functions.GetPlayerData().metadata["isdead"] or QBCore.Functions.GetPlayerData().metadata["ishandcuffed"] then return end
					...
					if distance < (closestDis or Config.MaxDistance) and distance <= zone.targetoptions.distance and zone:isPointInside(coords) then
						...
						CreateThread(function()
							while isTargeting do
								...
								DrawSprite("shared", "emptydot_32", 0, 0, 0.02, 0.035, 0, 255, 85, 30, 255.0)
							end
						end)
					end
				end
		...
		CreateThread(function()
			...
			if data then
				...
				elseif data.event then
					...
					print("Tetikleyici kurulumu yok")
				end
			end
		end)
	end
...
CreateThread(function()
    ...
    RegisterKeyMapping("+playerTarget", "Hedeflemeyi etkinleştir", "keyboard", Config.OpenKey)
