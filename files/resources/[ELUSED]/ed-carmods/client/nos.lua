...
function GetNos()
	local p = promise.new()
	QBCore.Functions.TriggerCallback('ed-carmods:GetNosLoaded', function(vehs) p:resolve(vehs) end)
...
function GetNosColour()
	local p = promise.new()
	QBCore.Functions.TriggerCallback('ed-carmods:GetNosColour', function(vehs) p:resolve(vehs) end)
...
RegisterNetEvent('ed-carmods:client:applyNOS', function()
	...
	if DoesEntityExist(vehicle) then
		...
		exports['qb-ui']:Circle(function(success)
			...
			if success then
				QBCore.Functions.Progressbar("drink_something", "NOS Takılıyor.", time, false, true, { disableMovement = true, disableCarMovement = true, disableMouse = false, disableCombat = false, },
				{ animDict = "mini@repair", anim = "fixing_a_ped", flags = 8, }, {}, {}, function() SetVehicleModKit(vehicle, 0)
					ClearPedTasks(playerPed)
					if VehicleNitrous[trim(GetVehicleNumberPlateText(vehicle))] then TriggerServerEvent("ed-carmods:server:giveNOS") end
					TriggerServerEvent('ed-carmods:server:LoadNitrous', trim(GetVehicleNumberPlateText(vehicle)))
					SetVehicleDoorShut(vehicle, 4, false)
					updateCar(vehicle)
					TriggerServerEvent('ed-carmods:server:removeNOS')
...
RegisterNetEvent('ed-carmods:client:UpdateNitroLevel', function(Plate, level) VehicleNitrous[Plate] = { hasnitro = true, level = level } end)

RegisterNetEvent('ed-carmods:client:LoadNitrous', function(Plate)
...
RegisterNetEvent('ed-carmods:client:UnloadNitrous', function(Plate)
	if not LocalPlayer.state.isLoggedIn then return end
    ...
    while true do
        ...
        if IsPedInAnyVehicle(PlayerPedId()) then
			...
			if VehicleNitrous[Plate] and VehicleNitrous[Plate].hasnitro then
				...
				if IsControlJustPressed(0, Config.NosBindings[1]) and GetPedInVehicleSeat(CurrentVehicle, -1) == PlayerPedId() then -- Purge/Nitro Mode swapping
					if purgemode then purgemode = false
						...
						else TriggerEvent("ed-carmods:client:DrawText", Loc[Config.Lan]["nos"].boostmode, tostring(Config.OdoLocation)) end
					else purgemode = true
						if not ShowOdo then TriggerEvent("QBCore:Notify", Loc[Config.Lan]["nos"].purgemode, "success")
						else TriggerEvent("ed-carmods:client:DrawText", Loc[Config.Lan]["nos"].purgemode, tostring(Config.OdoLocation)) end
					end
				end
				if IsControlJustPressed(0, Config.NosBindings[3]) and GetPedInVehicleSeat(CurrentVehicle, -1) == PlayerPedId() then -- UP
					if purgemode and purgeSize < 1.0 then
						...
						if purgeSize > 0.1 then
							...
							else TriggerEvent("ed-carmods:client:DrawText", Loc[Config.Lan]["nos"].spray..math.floor(purgeSize*10), tostring(Config.OdoLocation)) end
						end
					end
					if not purgemode and boostLevel < 3 and not NitrousActivated then
						...
						if boostLevel <= 3 then
							...
							else TriggerEvent("ed-carmods:client:DrawText", Loc[Config.Lan]["nos"].boost..boostLevel, tostring(Config.OdoLocation)) end
						end
					end
				end
				...
				if IsControlJustPressed(0, Config.NosBindings[2]) and GetPedInVehicleSeat(CurrentVehicle, -1) == PlayerPedId() then
					forceStop = true
					if purgemode then
						TriggerServerEvent('ed-carmods:server:SyncPurge', VehToNet(CurrentVehicle), true)
						CreateThread(function()
							if boosting then
					...
					if not purgemode then
						if (GetEntitySpeed(CurrentVehicle) * 3.6) > 25.0 and (GetVehicleWheelieState(CurrentVehicle) ~= 129 and GetVehicleWheelieState(CurrentVehicle) ~= 65 and not IsEntityInAir(CurrentVehicle)) and not boosting then
							...
							if Config.EnableTrails then TriggerServerEvent('ed-carmods:server:SyncTrail', VehToNet(CurrentVehicle), true) end
							...
							CreateThread(function()
								while NitrousActivated do
									if VehicleNitrous[Plate].level - 1 >= 0 then
										...
										if Config.EnableFlame then TriggerServerEvent('ed-carmods:server:SyncFlame', VehToNet(CurrentVehicle), true) end
										TriggerServerEvent('ed-carmods:server:UpdateNitroLevel', Plate, (VehicleNitrous[Plate].level - nitrousUseRate))
									...
									elseif VehicleNitrous[Plate].level - 1 <= 0 then
										TriggerServerEvent('ed-carmods:server:UnloadNitrous', Plate)
										TriggerServerEvent("ed-carmods:server:giveNOS")
										if Config.EnableScreen then SetNitroBoostScreenEffectsEnabled(false) end
										if Config.EnableTrails then TriggerServerEvent('ed-carmods:server:SyncTrail', VehToNet(CurrentVehicle), false) end
										NitrousActivated = false
										boosting = not boosting
										SetVehicleBoostActive(CurrentVehicle, 0)
										if Config.EnableFlame then TriggerServerEvent('ed-carmods:server:SyncFlame', VehToNet(CurrentVehicle), false) end
				...
				if (IsControlJustReleased(0, Config.NosBindings[2]) or IsControlJustPressed(0, 18)) and GetPedInVehicleSeat(CurrentVehicle, -1) == PlayerPedId() then
					TriggerServerEvent('ed-carmods:server:SyncPurge', VehToNet(CurrentVehicle), false)
			...
			if forceStop then -- This will make sure it doesn't break boosting in any car the person leaves. Clears the effects once and waits till you boost in another car
				...
				TriggerServerEvent('ed-carmods:server:SyncPurge', VehToNet(GetVehiclePedIsIn(PlayerPedId(), true)), false)
				if Config.EnableFlame then TriggerServerEvent('ed-carmods:server:SyncFlame', VehToNet(GetVehiclePedIsIn(PlayerPedId(), true)), false) end
				...
				if VehicleNitrous[trim(GetVehicleNumberPlateText(GetVehiclePedIsIn(PlayerPedId(), true)))] then
					...
					NitrousActivated = false
					boostLevel = 1
				if Config.EnableTrails then TriggerServerEvent('ed-carmods:server:SyncTrail', VehToNet(GetVehiclePedIsIn(PlayerPedId(), true)), false) end
				if Config.EnableScreen then SetNitroBoostScreenEffectsEnabled(false) end
				damageTimer = 0
				purgeCool = 0
				boosting = false
...
RegisterNetEvent('ed-carmods:client:SyncPurge', function(netid, enabled)
	if not LocalPlayer.state.isLoggedIn then return end
...
RegisterNetEvent('ed-carmods:client:SyncTrail', function(netid, enabled)
	if not LocalPlayer.state.isLoggedIn then return end
	if #(GetEntityCoords(NetToVeh(netid)) - GetEntityCoords(PlayerPedId())) >= 200 then return end
...
RegisterNetEvent('ed-carmods:client:SyncFlame', function(netid, enable)
	if not LocalPlayer.state.isLoggedIn then return end
	if #(GetEntityCoords(NetToVeh(netid)) - GetEntityCoords(PlayerPedId())) >= 200 then return end
	if enable then
...
-- Mechanic Remove NOS
RegisterNetEvent('ed-carmods:client:giveNOS', function()
	local playerPed	= PlayerPedId()
	local coords = GetEntityCoords(playerPed)
	...
	{ animDict = "mini@repair", anim = "fixing_a_ped", flags = 16, }, {}, {}, function()
		...
		SetVehicleDoorShut(vehicle, 4, false)
		updateCar(vehicle)
		TriggerServerEvent('ed-carmods:server:UnloadNitrous', trim(GetVehicleNumberPlateText(vehicle)))
		TriggerServerEvent('ed-carmods:server:giveNOS')
		emptyHands(playerPed)
...
RegisterNetEvent('ed-carmods:client:ChangeColour', function(Plate, newColour)
	if not LocalPlayer.state.isLoggedIn then return end
...
RegisterNetEvent('ed-carmods:client:NOS:RGBApply', function(data)
	...
	Wait(3000)
	TriggerServerEvent("ed-carmods:server:ChangeColour", trim(GetVehicleNumberPlateText(vehicle)), data)
	SetVehicleDoorShut(vehicle, 4, false)
	emptyHands(ped)
	if Config.CosmeticRemoval then TriggerServerEvent("ed-carmods:RemoveItem", 'noscolour', 1) TriggerEvent('inventory:client:ItemBox', QBCore.Shared.Items['noscolour'], "remove", 1) end
	TriggerEvent("QBCore:Notify", Loc[Config.Lan]["nos"].nosColour.." "..Loc[Config.Lan]["nos"].tireinst, "success")
	qblog("`noscolour` installed [**"..trim(GetVehicleNumberPlateText(vehicle)).."**]")
end)

RegisterNetEvent('ed-carmods:client:NOS:RGBPicker', function()
	...
	if DoesEntityExist(vehicle) then
		...
		if dialog then
			...
			local data = { r, g, b }
			TriggerEvent('ed-carmods:client:NOS:RGBApply', data)
		end
    end
end)

RegisterNetEvent('ed-carmods:client:NOS:HEXPicker', function()
	local coords = GetEntityCoords(PlayerPedId())
	local vehicle = getClosest(coords)
	local currentCol
	local plate = trim(GetVehicleNumberPlateText(vehicle))
	...
	if DoesEntityExist(vehicle) then
		...
		if dialog then
			if not dialog.hex then return TriggerEvent("ed-carmods:client:NOS:HEXPicker") end
			if dialog.hex then
				local hex = dialog.hex:gsub("#","")
				while string.len(hex) < 6 do hex = hex.."0"	Wait(10) end
				TriggerEvent('ed-carmods:client:NOS:RGBApply', table.pack(HexTorgb(hex)))
			end
		end
    end
end)

RegisterNetEvent('ed-carmods:client:NOS:rgbORhex', function()
	local ped = PlayerPedId()
	local coords = GetEntityCoords(ped)
	local vehicle = getClosest(coords)
	if not inCar() then return end
	...
	if DoesEntityExist(vehicle) then
		...
		PaintMenu[#PaintMenu+1] = { icon = "fa fa-mail-reply", header = "Kapat", params = { event = "ed-carmods:client:Menu:Close" } }
		PaintMenu[#PaintMenu+1] = { header = Loc[Config.Lan]["paintrgb"].hex, text = Loc[Config.Lan]["common"].current..":<br>"..currentHEXCol, params = { event = "ed-carmods:client:NOS:HEXPicker" }, }
		PaintMenu[#PaintMenu+1] = { header = Loc[Config.Lan]["paintrgb"].rgb, text = Loc[Config.Lan]["common"].current..":<br>"..currentRBGCol, params = { event = "ed-carmods:client:NOS:RGBPicker" }, }
