...
CreateThread(function()
    ...
    AddTextComponentString('Hayvan Satıcısı')
...
Citizen.CreateThread(function()
    while true do
        ...
        local spawnCoords = vector3(563.5, 2753.23, 41.88)
        local dst = #(spawnCoords - pedCoords)

        if dst < 100 and petShopSpawned == false then
            TriggerEvent('nc-petshop:spawnPed', spawnCoords, 184.48)
            petShopSpawned = true
        end
        if dst >= 101 then
...
Citizen.CreateThread(function()
    while true do
        Citizen.Wait(1000)
        local pedCoords = GetEntityCoords(PlayerPedId())
        local spawnCoords = vector3(559.77, 2746.62, 41.88)
        local dst = #(spawnCoords - pedCoords)

        if dst < 100 and vetSpawned == false then
            TriggerEvent('nc-petshop:spawnVet', spawnCoords, 277.9)
            vetSpawned = true
        end
        if dst >= 101 then
            vetSpawned = false
...
AddEventHandler('nc-petshop:spawnVet', function(coords, heading)
    ...
    exports['qb-target']:AddEntityZone('vet', vet, {
        ...
        options = {{
            ...
            icon = "fa fa-circle",
            label = "Hayvanı Tedavi Et"
        }, {
            type = "client",
            event = "nc-petshop:removePet",
            icon = "fa fa-circle",
            label = "Hayvanı Özgür Bırak"
        }},
        distance = 2.5
    })
...
AddEventHandler('nc-petshop:spawnPet', function(modelname)
    ...
    if DoesEntityExist(pet) then
        ...
        elseif petModel == GetHashKey('a_c_mtlion') then
            ...
            SleepDict = nil
            SleepAnim = nil
        elseif petModel == GetHashKey('a_c_poodle') then
            SitDict = "creatures@retriever@amb@world_poodle_sitting@idle_a"
            SitAnim = "idle_b"
            BarkDict = "creatures@rottweiler@amb@world_poodle_barking@idle_a"
            BarkAnim = "idle_a"
            SleepDict = "creatures@rottweiler@amb@sleep_in_kennel@"
            SleepAnim = "sleep_in_kennel"
...
AddEventHandler('nc-petshop:removePet', function()
    QBCore.Functions.Notify('Hayvanın Artık Özgür!') 
...
AddEventHandler('nc-petshop:healPet', function()
    ...
    if DoesEntityExist(pet) then
        ...
        QBCore.Functions.Notify('Hayvanınız Tedavi Ediliyor!')
        Wait(15000)
        stay = false
        ...
        if petHealth < 100 or petSpeed ~= 10.0 then
            if illness == 'fatigue' then
                QBCore.Functions.Notify('Hayvanınız Tedavi Edildi ' .. illness)
                SetEntityHealth(pet, 200)
                illness = 'none'
                illnessFee = 100
                petSpeed = 10.0
            elseif illness == 'kennelcough' then
                QBCore.Functions.Notify('Hayvanınız Tedavi Edildi! ' .. '')
                illness = 'none'
                SetEntityHealth(pet, 200)
                illnessFee = 300
                petHealth = 100
        ...
        QBCore.Functions.Notify('Tedavi Edebilceğin Hayvanın Yok!')
    end
end)
...
AddEventHandler('nc-petshop:feedPet', function()
    QBCore.Functions.TriggerCallback("nc-petshop:feedPetCallback", function(result)
        if result then
            ...
            QBCore.Functions.Notify("Hayvan yemin yok!. Git hayvan dükkanından yem al!", "error")
        end
    end)
...
AddEventHandler('nc-petshop:petStore', function()
    local petMenu = {{
        isMenuHeader = true,
        header = "Hayvan Dükkanı"
    }, {
        header = "Hayvan Yemi",
        txt = "Satın Al" .. "  $" .. Config.FoodPrice,
        params = {
