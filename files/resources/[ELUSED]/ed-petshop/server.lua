...
AddEventHandler('nc-petshop:buyPet', function(petType,price)
	...
	exports.oxmysql:execute('SELECT * FROM pets WHERE owner = ?', {
		...
		if ifOwner ~= nil then
			TriggerClientEvent('QBCore:Notify', _source, 'Zaten Hayvanın Var!', 'error')
			print('En Fazla 1 Tane Hayvan Alabilirsin!') -- Add notification here
		else
			if xPlayer.PlayerData.money.cash > price then
				TriggerClientEvent('QBCore:Notify', _source, 'Hayvan Satın Aldın!', 'success')
				print('Hayvanı Satın Aldın!') -- Add notification here
				...
				TriggerClientEvent('QBCore:Notify', _source, 'Hayvanı Alacak Paran Yok!', 'error')
				print('Hayvanı Alacak Paran Yok!') -- Add notification here
			end
		end
	end)
end)
