...
local function grabWebhook()
    ...
    TriggerServerEvent("ed-kamera:grabHook", Key)
        ...
        while camera and not IsEntityDead(lPed) and (GetVehiclePedIsIn(lPed) == vehicle) do
            ...
            elseif IsControlJustPressed(1, 176) then
                ...
                exports['screenshot-basic']:requestScreenshotUpload(tostring(hook), "files[]", function(data)
                    ...
                    if cameraprop then DeleteEntity(cameraprop) end
                    ClearPedTasks(lPed)
                    TriggerServerEvent("ed-kamera:CreatePhoto", json.encode(image.attachments[1].proxy_url))
...
RegisterNetEvent("ed-kamera:usePhoto", function(url)
    photo = not photo
...
RegisterNetEvent('ed-kamera:useCamera', function()
    camera = not camera
