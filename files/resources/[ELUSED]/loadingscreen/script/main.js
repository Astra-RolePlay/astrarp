...
window.addEventListener("keyup", function (event) {
  if (event.which == 37) {
    ...
    if (document.getElementById("loading").src.endsWith("song2.mp3")) {
      ...
      document.getElementById("loading").src = "song/song3.mp3";
      songname.innerHTML =
        "Wiguez & Vizzen Ft. Maestro Chives - Running Wild (EH!DE Remix) [NCS Release]";
    } else if (document.getElementById("loading").src.endsWith("song3.mp3")) {
      document.getElementById("loading").src = "song/song2.mp3";
      songname.innerHTML = "Akacia - Electric [NCS Release]";
    }
    document.getElementById("loading").play();
    mutetext.innerHTML = "Müziği Sustur";
  }
...
window.addEventListener("DOMContentLoaded", () => {
  ...
  document.querySelector("#namePlaceholder > span").innerText =
...
(function welcometext() {
  var welcomes = [
    "Heyecan dolu yeni macerana başla.",
    "Yeni şehrin güzelliklerini keşfet.",
    "Yepyeni bir bölümün kapısını arala.",
    "Yeni olanaklar dünyasına adım at.",
    "Taze başlangıcını kucakla.",
  ];
