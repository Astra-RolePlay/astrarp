        ...
        if v.UseTarget then
            ...
            if not pedExists then
                ...
                exports[Config.Target]:AddTargetEntity(ped, {
                    options = {
                        { 
                            event = "elused-policegarage:OpenMainMenu",
                            icon = Config.Locals['Targets']['GarageTarget']['Icon'],
                            label = Config.Locals['Targets']['GarageTarget']['Label'],
                            rjob = v.JobRequired,
                            userent = v.UseRent,
                    ...
                    if distance < 10 then
                        ...
                        if distance < 1.5 then
                            ...
                            if IsControlJustReleased(0, 38) then
                                ...
                                TriggerEvent("elused-policegarage:OpenMainMenu", Data)
                            end
                        end
                    end
                end
...
RegisterNetEvent('elused-policegarage:OpenMainMenu', function(data)
    local MainMenu = {
        {
    ...
    if data.usepurchasable then
        table.insert(MainMenu, {
            header = "Araçlar",
            txt = "Tüm Araçlar Ücretsizdir.",
            icon = "fa-solid fa-money-check-dollar",
            params = {
                event = "elused-policegarage:OpenPurchaseMenu",
                args = {
    ...
    if IsPedInAnyVehicle(PlayerPedId(), false) and data.useliveries then
        table.insert(MainMenu, {
            ...
            params = {
                event = "elused-policegarage:StartSelection",
                args = {
    ...
    if IsPedInAnyVehicle(PlayerPedId(), false) and data.useextras then
        table.insert(MainMenu, {
            ...
            params = {
                event = "elused-policegarage:OpenExtrasMenu",
                args = {
                    vehicle = GetVehiclePedIsIn(PlayerPedId(), false),
                    station = data.station,
                    userent = data.userent,
    ...
    if PlayerRentedVehicle[PlayerPedId()] and PlayerRentedVehicle[PlayerPedId()].station == data.station and data.userent and GetVehiclePedIsIn(PlayerPedId(), false) == PlayerRentedVehicle[PlayerPedId()].vehicle then
        table.insert(MainMenu, {
            ...
            params = {
                event = "elused-policegarage:ReturnRentedVehicle",
            }
        })
    end
    table.insert(MainMenu, {
...
RegisterNetEvent("elused-policegarage:OpenExtrasMenu", function(data)
    local ExtrasMenu = {
        {
        ...
        if DoesExtraExist(data.vehicle, i) then
            ...
            if IsVehicleExtraTurnedOn(data.vehicle, i) then
                table.insert(ExtrasMenu, {
                    ...
                    icon = "fa-solid fa-xmark",
                    params = {
                        event = "elused-policegarage:VehicleExtra",
                        args = {
...
RegisterNetEvent("elused-policegarage:OpenRentingMenu", function(data)
        ...
        if not PlayerRentedVehicle[PlayerPedId()] then
            for k, v in pairs(data.rentvehicles) do
                table.insert(RentingMenu, {
                    header = "Kirala " .. k,
                    txt = "Kirala: " .. k .. "<br> Ücret: " .. v.PricePerMinute .. "$ (Dakika)",
                    icon = "fa-solid fa-car",
                    params = {
                        event = "elused-policegarage:ChooseRent",
                        args = {
...
RegisterNetEvent("elused-policegarage:OpenPurchaseMenu", function(data)
    local VehicleMenu = {
        {
            header = "POLICE - GARAGE",
            icon = "fa-solid fa-circle-info",
    ...
    for i = 1, #sortedVehicles do
        ...
        table.insert(VehicleMenu, {
            header = " " .. k,
            txt = "Araçı Al: " .. k .. "<br> Fiyat: " .. priceText,
            icon = "fa-solid fa-circle-check",
            params = {
                event = "elused-policegarage:StartPreview",
                args = {
                    price = v.TotalPrice,
                    vehiclename = k,
                    vehicle = v.Vehicle,
...
RegisterNetEvent("elused-policegarage:OpenNearMenu", function(data)
    local PlayersMenu = {
        {
    ...
    table.insert(PlayersMenu, {
        ...
        params = {
            isServer = true,
            event = "elused-policegarage:BuyVehicle",
            args = {
    ...
    if not GetNearbyPlayers(PlayerPedId(), Config.CompanyFunds['CheckDistance'], data.job) then
        ...
        for k, v in pairs(GetNearbyPlayers(PlayerPedId(), Config.CompanyFunds['CheckDistance'], data.job)) do
            table.insert(PlayersMenu, {
                header = v.name,
                txt = "Araçı Al " .. data.vehiclename .. " for " .. v.name,
                icon = "fa-solid fa-user-check",
                params = {
                    isServer = true,
                    event = "elused-policegarage:BuyVehicle",
    ...
    table.insert(PlayersMenu, {
        ...
        icon = "fa-solid fa-left-long",
        params = {
            event = "elused-policegarage:ChoosePayment",
            args = {
                price = data.price,
                vehiclename = data.vehiclename,
                vehicle = data.vehicle,
...
RegisterNetEvent("elused-policegarage:SpawnRentedVehicle", function(vehicle, vehiclename, amount, time, realtime, spawncoords, paymenttype, job, station)
...
RegisterNetEvent("elused-policegarage:SpawnPurchasedVehicle", function(vehicle, spawncoords, checkradius, job, useownable, trunkitems, extras, liveries, station)
    QBCore.Functions.SpawnVehicle(vehicle, function(veh)
        SetVehicleNumberPlateText(veh, FormatString(station)..tostring(math.random(1000, 9999)))
        exports[Config.FuelSystem]:SetFuel(veh, 100.0)
        ...
        if useownable then
            TriggerServerEvent("elused-policegarage:AddData", "vehiclepurchased", vehicle, GetHashKey(veh), QBCore.Functions.GetPlate(veh), job)
        end
    end, spawncoords, true)
end)

RegisterNetEvent("elused-policegarage:ReturnRentedVehicle", function()
	local player = PlayerPedId()
    ...
    if IsPedInAnyVehicle(player, false) then
        QBCore.Functions.TriggerCallback('elused-policegarage:GetRealTime', function(result)
            if not PlayerRentedVehicle[player] then
            ...
            TriggerServerEvent("elused-policegarage:RefundRent", PlayerRentedVehicle[player].paymenttype, refund, GetPlayerServerId(PlayerId()), PlayerRentedVehicle[player].job)
...
RegisterNetEvent("elused-policegarage:StartSelection", function(data)
    local player = PlayerPedId()
...
RegisterNetEvent("elused-policegarage:VehicleExtra", function(data)
    local Data = {
        userent = data.userent,
        rentvehicles = data.rentvehicles,
        purchasevehicles = data.purchasevehicles,
    ...
    if IsVehicleExtraTurnedOn(data.vehicle, data.extraid) then
        ...
        TriggerEvent("elused-policegarage:OpenExtrasMenu", Data)
    else
...
RegisterNetEvent("elused-policegarage:StartPreview", function(data)
    local player = PlayerPedId()
    if QBCore.Functions.SpawnClear(vector3(data.coordsinfo['PreviewSpawn'].x, data.coordsinfo['PreviewSpawn'].y, data.coordsinfo['PreviewSpawn'].z), data.coordsinfo['CheckRadius']) then
            ...
            QBCore.Functions.SpawnVehicle(data.vehicle, function(veh)
                ...
                Citizen.CreateThread(function()
                    while IsCamActive(VehicleCam) do
                        ShowHelpNotification("Aracı Satın Almak İçin [E] Tuşuna Basın Satın Almayı İptal Etmek İçin [BACKSPACE] Tuşuna Basın.")
                        if IsControlJustReleased(0, 177) then
                        ...
                        if IsControlJustReleased(0, 38) then
                            ...
                            TriggerEvent("elused-policegarage:ChoosePayment", VehicleData)
                            break
                        end
                        Citizen.Wait(1)
                    end
...
RegisterNetEvent("elused-policegarage:ChooseRent", function(data)
    ...
    if minutes ~= nil then
        ...
        if minutesamount > 0 and minutesamount <= Config.RentMaximum then
            local paymentType = exports["qb-input"]:ShowInput({
                header = "Ödeme Yöntemini Seçiniz",
                submitText = "Seç",
                inputs = {
                    {
                        text = 'Ödeme Yöntemi',
            ...
            if paymentType ~= nil then
                ...
                if price ~= nil then
                    TriggerServerEvent("elused-policegarage:RentVehicle", paymentType.paymenttype, finalPrice, data.vehiclename, data.vehicle, minutes.minutes, data.coordsinfo, data.job, data.station)
                end
            end
        else
...
RegisterNetEvent("elused-policegarage:ChoosePayment", function(data)
    ...
    if paymentType ~= nil then
        ...
        if price ~= nil and paymentType.paymenttype == "company" and PlayerJob.isboss and Config.CompanyFunds['Enable'] then
            TriggerEvent("elused-policegarage:OpenNearMenu", {
            ...
            if price ~= nil then
                TriggerServerEvent("elused-policegarage:BuyVehicle", {
                    paymenttype = paymentType.paymenttype, 
                    price = data.price,
                    vehiclename = data.vehiclename,
                    vehicle = data.vehicle,
