...
local function spawnObj(model, coords, heading)
    ...
    exports['qb-target']:AddTargetEntity(object, {
        options = { {
             ...
             label = "Üret",
 --            job = "police",  Meslekli yapmak için çizgileri silin ve police yerine mesleğin adını yazın
             action = function()
                TriggerServerEvent("ed-crafting_sv:getWorkBenchData")
             end
...
RegisterNUICallback("discardBlueprint", function(data, cb)
    if currentBenchId then
        QBCore.Functions.TriggerCallback('ed-crafting_sv:discardBlueprint', function (result)
            cb(result)
...
RegisterNUICallback("attemptCraft", function(data, cb)
    if currentBenchId then
        TriggerServerEvent("ed-crafting_sv:attemptCraft", currentBenchId, data.currentRecipe, data.amt, data.isBlueprintRecipe)
    end
end)
...
RegisterNetEvent("ed-crafting_cl:openCraftingBench", function(craftingBenchData, benchId)
...
RegisterNetEvent("ed-crafting_cl:increasedRep", function(craftingRep, attachmentRep)
    ...
    if #recipes > #currentDefaultRecipes then
        ...
        QBCore.Functions.Notify('Yeni tarifin kilidi açıldı', 'success')
    end
end)
