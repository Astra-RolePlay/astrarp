	...
	if (QBCore.Functions.GetPlayerData().metadata["isdead"]) or (QBCore.Functions.GetPlayerData().metadata["inlaststand"]) and spam then
		QBCore.Functions.TriggerCallback('hhfw:docOnline', function(EMSOnline, hasEnoughMoney)
			if EMSOnline <= Config.Doctor and hasEnoughMoney and spam then
				...
				Notify("Doktor geliyor")
			else
				if EMSOnline > Config.Doctor then
					Notify("Şu anda aktif doktorlar var!", "error")
				elseif not hasEnoughMoney then
					Notify("Paran yetmiyor fakir", "error")
				else
					Notify("Doktor geliyor", "primary")
				end	
			end
		end)
	else
		Notify("Bu komutu sadece baygınken kullanabilirsin.", "error")
	end
end)
...
function SpawnVehicle(x, y, z)  
	spam = false
	local vehhash = GetHashKey("sovereign")                                                     
...
function DoctorNPC()
	...
	QBCore.Functions.Progressbar("revive_doc", "Tedavi oluyorsun", Config.ReviveTime, false, false, {
		...
		Notify("Tedavin bitti doktora ödediğin ücret: "..Config.Price, "success")
