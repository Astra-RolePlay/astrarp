    ...
    while true do
        ...
        for i, v in ipairs(Locations) do
            ...
            if dist < 1.5 and IsPedInAnyVehicle(ped, false) and not washing then
                drawNativeNotification("~INPUT_CONTEXT~ Aracı Yıkat")
                esta = true          
                sleep = 0
...
RegisterNUICallback("wash", function(data)
    ...
    trigger('buty:getMoney', function (money)
        if money then
            QBCore.Functions.Notify("Doğru ödeme yaptınız, aracınızı temizlemelerini bekleyin.", "success")
            if Type == "1" then
                for _, location in ipairs(Locations) do
                    ...
                    if dist < 20 then
                        for _, npcData in ipairs(location.Npc['BASIC']) do             
                            ...
                            Progress(24000, "Aracınız Temizleniyor.") 
                            Wait(2000) 
                        ...
                        for _, npcData in ipairs(location.Npc['STANDARD']) do             
                            for i = 1, 3 do
                                ...
                                elseif done == 2 then
                                    ...
                                    RenderScriptCams(true, true, 5000, true, false)  
                                    Wait(2000)    
                                    Progress(50000, "Özel Dans Sergileniyor.")  
                                end                      
...
Progress = function(time, text)
    QBCore.Functions.Progressbar("temizle", text, time, false, true, {
