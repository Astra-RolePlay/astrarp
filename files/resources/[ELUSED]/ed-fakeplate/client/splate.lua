...
RegisterNetEvent('ed-fakeplate:splate-plug')
AddEventHandler('ed-fakeplate:splate-plug', function()
    if not cooldown then
        ...
        if IsPedInAnyVehicle(player, false) then
            ...
            if vehSeed < 5 then
                local progbar = true
                QBCore.Functions.TriggerCallback('ed-fakeplate:plate-control', function(owned, text)
                    if owned then
                        QBCore.Functions.Progressbar("plaka_degistir", Lang:t('progress.changeplate'), 15000, false, true, { -- p1: menu name, p2: yazı, p3: ölü iken kullan, p4:iptal edilebilir
                            ...
                            QBCore.Functions.TriggerCallback('ed-fakeplate:plate-plug', function(status)
                                if status then
...
RegisterNetEvent('ed-fakeplate:splaka-shock')
AddEventHandler('ed-fakeplate:splaka-shock', function()
    local player = PlayerPedId()
    local closestVehicle = GetVehiclePedIsIn(player, false)
    ...
    if not cooldown then
        ...
        if IsPedInAnyVehicle(player, false) then
            ...
            if vehSeed < 5 then
                local progbar = true
                QBCore.Functions.TriggerCallback('ed-fakeplate:fake-plate-control', function(plaka)
                    if plaka then
                        QBCore.Functions.Progressbar("plaka_degistir", Lang:t('progress.plateremoving'), 5000, false, true, { -- p1: menu name, p2: yazı, p3: ölü iken kullan, p4:iptal edilebilir
                            ...
                            TriggerServerEvent("ed-fakeplate:s-plate-shock", splaka)
