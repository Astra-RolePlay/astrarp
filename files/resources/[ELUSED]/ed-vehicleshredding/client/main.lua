	...
	for k,v in pairs(Config.Places) do
        ...
        exports['qb-target']:AddCircleZone("carpartibus_sales", vector3(v.tped.x, v.tped.y, v.tped.z), 2.0,{
            ...
            options = {
                {
                    type = "client",
                    event = "ed-vehicleshredding:client:openMenu",
					icon = "fas fa-shop",
                    label = label,
                },
            },
						...
						if distance < 1.2 then 
							...
							if IsControlJustPressed(0, 46) and not clientcooldown then
								if CurrentCops >= Config.MinimumCops then
									StartChopThisCar()
									Citizen.Wait(35000)
									exports['elused-dispatch']:aracparcalama()
								else
...
RegisterNetEvent('ed-vehicleshredding:client:openMenu', function()
            local chopShop = {
                {
                ...
                },
                {
                    ...
                    params = {
                        event = 'ed-vehicleshredding:client:openChop',
                        args = {
...
RegisterNetEvent('ed-vehicleshredding:client:openChop', function(data)
    QBCore.Functions.TriggerCallback('ed-vehicleshredding:server:getInv', function(inventory)
            ...
            for i = 1, #data.items do
                if v.name == data.items[i].item then
                    chopMenu[#chopMenu + 1] = {
                        ...
                        params = {
                            event = 'ed-vehicleshredding:client:chopitems',
                            args = {
...
RegisterNetEvent('ed-vehicleshredding:client:chopitems', function(item)
    local sellingItem = exports['qb-input']:ShowInput({
     ...
     if sellingItem then
        ...
        if tonumber(sellingItem.amount) > 0 then
            TriggerServerEvent('ed-vehicleshredding:server:sellItems', item.name, sellingItem.amount, item.price)
        else
...
function FinishedChopping()
	carDismantlingActive = false
	DeleteEntity(car)
	TriggerServerEvent("ed-vehicleshredding:giveitem", class)
end
