...
-- System Infos
PerformHttpRequest(DiscordWebhookSystemInfos, function(Error, Content, Head) end, 'POST', json.encode({username = SystemName, content = '**Sunucu Aktif!**'}), { ['Content-Type'] = 'application/json' })

AddEventHandler('playerConnecting', function()
	TriggerEvent('ed-logs:ToDiscord', DiscordWebhookSystemInfos, SystemName, '```css\n' .. GetPlayerName(source) .. ' Sunucuya Giriş Yapıyor\n```', SystemAvatar, false)
end)

AddEventHandler('playerDropped', function(Reason)
	TriggerEvent('ed-logs:ToDiscord', DiscordWebhookSystemInfos, SystemName, '```fix\n' .. GetPlayerName(source) .. ' Sunucudan Ayrıldı (' .. Reason .. ')\n```', SystemAvatar, false)
end)

-- Killing Log
RegisterServerEvent('ed-logs:playerDied')
AddEventHandler('ed-logs:playerDied', function(Message, Weapon)
	...
	TriggerEvent('ed-logs:ToDiscord', DiscordWebhookKillinglogs, SystemName, Message .. ' `' .. date.day .. '.' .. date.month .. '.' .. date.year .. ' - ' .. date.hour .. ':' .. date.min .. ':' .. date.sec .. '`', SystemAvatar, false)
end)
			...
			PerformHttpRequest('http://steamcommunity.com/profiles/' .. tonumber(GetIDFromSource('steam', Source), 16) .. '/?xml=1', function(Error, Content, Head)
				...
				for i, Line in ipairs(SteamProfileSplitted) do
					if Line:find('<avatarFull>') then
						...
						TriggerEvent('ed-logs:ToDiscord', Webhook, Name .. ' [ID: ' .. Source .. ']', Message, AvatarURL, false, Source, TTS) --Sending the message to discord
						break
					end
				end
			end)
...
RegisterServerEvent('ed-logs:ToDiscord')
AddEventHandler('ed-logs:ToDiscord', function(WebHook, Name, Message, Image, External, Source, TTS)
	...
	if External then
		...
		elseif not Webhook:find('discord.com/api/webhooks') then
			print('ed-logs event called without a specified webhook!')
			return nil
		end
