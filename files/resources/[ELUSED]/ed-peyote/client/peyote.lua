...
local function SpawnPeyotePlants()
	...
	while spawnedPeyote < 7 do
		...
		particles = StartParticleFxLoopedAtCoord(light, PeyoteCoords.x, PeyoteCoords.y, PeyoteCoords.z, 0.0, 0.0, 0.0,
			0.02, false, false, false, false)
...
RegisterNetEvent('ed-peyote:proccessing', function()
...
RegisterNetEvent("ed-peyote:pickPeyote", function()
	...
	if nearbyObject and IsPedOnFoot(playerPed) then
		if not isPickingUp then
			...
			QBCore.Functions.Progressbar("peyote_topla", Lang:t("peyote.progress"), 1300, false, true, {
				...
				spawnedPeyote -= 1
				TriggerServerEvent('ed-peyote:pickedUpPeyote')
				isPickingUp = false
			end, function()
				ClearPedTasks(PlayerPedId())
				isPickingUp = false
