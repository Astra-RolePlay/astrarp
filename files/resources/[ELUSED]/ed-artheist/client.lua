                ...
                if dist <= 1.0 then
                    ...
                    if not v['taken'] then
                        ...
                        if IsControlJustPressed(0, 38) then
                            ...
                            if weapon == GetHashKey('WEAPON_KNIFE') then
                                if not ArtHeist['cuting'] then
                                    HeistAnimation(k)
                                else
                                    QBCore.Functions.Notify('Zaten hırsızlık yapıyorsun.', 'error', 3500)
                                end
                            else
                                QBCore.Functions.Notify('Bıçağa ihtiyacın var.', 'error', 3500)
                            end
                            end
                        end)
                        end
...
function StartHeist()
    QBCore.Functions.TriggerCallback('artheist:server:checkRobTime', function(time)
        if time then
            if ArtHeist['start'] then QBCore.Functions.Notify('Zaten soyguna başladınız. Bitene kadar bekleyin.', 'error', 3500) return end
            ...
            QBCore.Functions.Notify('Madrazo Malikanesi\'ne gidin ve tabloyu çalın.', 'success', 3500)
...
function HeistAnimation(sceneId)
    ...
    if ArtHeist['cut'] == #Config['ArtHeist']['painting'] then
        ...
        QBCore.Functions.Notify('Konum\'a gidin ve tabloları satın.', 'success', 3500)
        RemoveBlip(stealBlip)
