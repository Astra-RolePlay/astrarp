...
RegisterServerEvent('ed-vending:server:attemptPurchase', function(item)
    local src = source
	local Player = QBCore.Functions.GetPlayer(src)
    local cash = Player.Functions.GetMoney('cash')
    if cash < item.price then
        TriggerClientEvent('ed-vending:client:failed', src)
    else
        Player.Functions.RemoveMoney("cash", item.price, "Vending Machine")
        Player.Functions.AddItem(item.name, 1)
        TriggerClientEvent('inventory:client:ItemBox', src, QBCore.Shared.Items[item.name], "add")
        TriggerClientEvent('ed-vending:client:success', item)
    end
end)


RegisterNetEvent('ed-vending:server:steal', function(machineItems)
...
RegisterNetEvent('ed-vending:server:removeLockpick', function()
    local Player = QBCore.Functions.GetPlayer(source)
    if not Player then return end
...
RegisterNetEvent('ed-vending:server:callCops', function(coords, streetLabel)
    local alertData = {
