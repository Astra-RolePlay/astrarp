...
$(document).on('click', "#submitFeedback", function() {
	...
	$.post('https://ed-report/action', JSON.stringify({
		action: "newFeedback",
		subject: subject,
...
function viewFeedbackWindow(feedbackid) {
	...
	if (data.category === 'player_report') {
		feedbackWcard.find(".feedback_category").html(`<span><i class="fas fa-user-alt"></i> Oyuncu Raporlama</span>`)
	} else if (data.category === 'question') {
		feedbackWcard.find(".feedback_category").html(`<span><i class="fas fa-question"></i> Soru</span>`)
	} else {
		feedbackWcard.find(".feedback_category").html(`<span><i class="fas fa-bug"></i> Hata</span>`)
	}
}
...
function reloadFeedbackTable() {
	...
	$.each(FeedbackTable, function(id, feedback) {
		...
		if (feedback.category === 'player_report') {
			category = "<span><i class='fas fa-user-alt'></i> Oyuncu Raporla</span>"
		} else if (feedback.category === 'question') {
			category = "<span><i class='fas fa-question'></i> Soru</span>"
		} else {
			category = "<span><i class='fas fa-bug'></i> Hata</span>"
		}
		...
		if (feedback.concluded != true) {
			feedbackTable.append(`
				<tr class="table-row" feedbackID="${id}">
					<td>Geri Bildirim #${feedback.feedbackid}</th>
					<td>${category}</td>
					<td style="color: ${statusColor};">${concluded}</td>
					<td><button type="button" class="btn btn-view"><i class="fas fa-eye"></i> İncele</button></td>
				</tr>`
			)
		}
	});
	...
	if ($(".feedbackTable .table").children().length <= 0) {
		$(".feedbackTable .feedback_subtitle").html("Rapor Başarılıyla Tamamlandı.");
		...
		$(".feedback_subtitle").html("Yeri Raporlar Mevcut.");
