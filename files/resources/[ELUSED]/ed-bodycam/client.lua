...
RegisterNetEvent("ed-bodycam:show", function(job, grade, name, src)
    ...
    if bodycam == false then
        ...
        TriggerServerEvent("ed-bodycam:server:setBodycamPlayers", BodycamPlayers)
...
RegisterNetEvent("ed-bodycam:client:setBodycamPlayers", function(PlayerPeds)
    BodycamPlayers = PlayerPeds
end)

RegisterNetEvent("ed-bodycam:close", function(src)
    bodycam = false
    closed = true
...
RegisterNetEvent('ed-bodycam:client:openCamera', function(src)
    ClearFocus()
    ExecuteCommand("e tablet")
    TriggerServerEvent("ed-bodycam:server:cullingRadius", 10000.0, src)
    Wait(2000)
...
RegisterNetEvent('ed-bodycam:client:exitCamera', function(src)
    exitCam()
end)
...
function exitCam()
    ...
    ExecuteCommand("e c")
    TriggerServerEvent("ed-bodycam:server:cullingRadius", 0.0, targetPedSource)
    Wait(2000)
    targetPedSource = nil
...
RegisterNetEvent("ed-bodycam:client:bodyCamera", function(job)
    openBodyCamMenu(job)
end)

function openBodyCamMenu(job)
    QBCore.Functions.TriggerCallback("ed-bodycam:client:getPlayers", function(players)
        local menu = {{
        ...
        for k, v in pairs(players) do
            menu[#menu + 1] = {
                ...
                params = {
                    event = "ed-bodycam:client:openCamera",
                    args = k
                }
            }
        end
